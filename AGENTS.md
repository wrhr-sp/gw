...
- 그룹웨어 UI 개발에서는 같은 기능을 하거나 같은 시각표현을 가진 요소는 공통토큰을 사용하는 것을 기본 원칙으로 한다. 버튼, 쓰기/저장/승인 같은 액션, 목록, 하위목록, 글꼴, 간격, 배지, 카드, 입력필드, 설정 컨트롤 등은 페이지별 예외 토큰보다 의미 기반 공통토큰을 우선 사용한다. 사용자가 명시적으로 예외 적용을 지시한 경우에만 기능/페이지별 예외 토큰 또는 별도 스타일을 둔다.
