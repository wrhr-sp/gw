...
describe("hotel daily sales migration", () => {
  ...
  it("enforces dynamic permissions, server totals, locked originals, owner-safe reads, and definer revocation", () => {
    ...
    expect(sql).toContain("revoke all on function public.hotel_daily_sales_command_v1");
    expect(sql).toContain("revoke all on function public.hotel_daily_sales_read_v1");
