...
const migrationUrl = new URL(
  "../migrations/0006_account_administration.sql",
  import.meta.url,
);
const readMigration = () => readFileSync(migrationUrl, "utf8");
const expandMigrationUrl = new URL(
  "../migrations/0009_global_login_id_expand.sql",
  import.meta.url,
);
const contractMigrationUrl = new URL(
  "../migrations/0010_global_login_id_contract.sql",
  import.meta.url,
);
const loginIdHistoryContractMigrationUrl = new URL(
  "../migrations/0023_login_id_registry_history_contract.sql",
  import.meta.url,
);
...
describe("account administration migration", () => {
  ...
  it("allows immutable registry history while keeping active login linkage exact", () => {
    const historyContract = readFileSync(
      loginIdHistoryContractMigrationUrl,
      "utf8",
    ).toLowerCase();
    expect(historyContract).toContain(
    ...
    expect(historyContract).not.toContain(
      "drop trigger login_id_registry_immutable",
    );
    expect(historyContract).not.toMatch(
      /delete\s+from\s+public\.login_id_registry/u,
    );
  });
  ...
  it("keeps EXPAND writer-compatible, then dead-letters legacy rows before CONTRACT linkage enforcement", () => {
    ...
    expect(expand).toContain(
      "attempt.id::text = job.payload->>'provisioningAttemptId'",
    );
    expect(expand).toContain(
      "attempt.target_user_id::text = job.payload->>'userId'",
    );
    expect(expand).toContain(
      "attempt.provider_subject = job.payload->>'providerSubject'",
    );
    expect(expand).not.toContain(
      "add constraint outbox_jobs_compensation_linkage_check",
    );
  ...
  it("persists provisioning state and compensation without password fields", () => {
    ...
    expect(sql).not.toMatch(
      /plain_password|password_hash|password_digest|new_password|password_value/i,
    );
    expect(sql).toContain(
      "public.jsonb_reject_plaintext_password_keys(completion_payload)",
    );
    expect(sql).toContain(
      "payload ?& array['userId', 'providerSubject', 'action']",
    );
  ...
  it("persists initial password change orchestration without credential derivatives", () => {
    ...
    expect(sql).not.toMatch(
      /initial_password_hash|new_password_hash|password_digest/i,
    );
  });
  ...
  it("forces tenant RLS for account lifecycle and provider outbox tables", () => {
    ...
    for (const [table, policy] of [
      [
        ...
        "account_provisioning_attempts_company_isolation",
      ],
      [
        "initial_password_change_attempts",
        "initial_password_change_attempts_company_isolation",
      ],
