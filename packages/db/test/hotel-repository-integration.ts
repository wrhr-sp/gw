...
try {
  ...
  const created = await repository.createHotel({
    ...ids("000000000001"),
    actor: { companyId, sessionId, userId: actorUserId, userType: "INTERNAL_STAFF" },
    branchId,
  ...
  if (created.status !== "CREATED" || created.hotel.status !== "PREPARING" || created.hotel.version !== 1) {
    throw new Error("hotel create did not return the persisted PREPARING hotel");
  }
  ...
  if (scopedList.status !== "OK"
    || scopedList.capabilities.canCreate
    || scopedList.hotels.length !== 1
    || scopedList.hotels[0]?.id !== branchId) {
    throw new Error("branch-scoped HOTEL_MANAGE did not expose the authorized hotel list");
  }
  const scopedDetail = await repository.getHotel(hotelScopedActor, branchId);
  if (scopedDetail?.id !== branchId) {
    throw new Error("branch-scoped HOTEL_MANAGE did not expose the authorized hotel detail");
  }
  await testSql`
    insert into permission_grants (
      id, company_id, branch_id, subject_type, subject_id, permission_code,
  ...
  `;
  const scopedDeniedList = await repository.listHotels(hotelScopedActor, undefined, {
  ...
  if (scopedDeniedList.status !== "FORBIDDEN") {
    throw new Error("branch deny did not remove the actor's only effective hotel scope");
  }
  ...
  `;
  if (scopedDeniedAudit[0]?.count !== 1) throw new Error("branch-only list denial was not audited");
  ...
  if (replay.status !== "REPLAYED" || replay.hotel.id !== branchId) {
    throw new Error("hotel create idempotency replay did not return the original hotel");
  }
  ...
  if (mismatch.status !== "IDEMPOTENCY_CONFLICT") throw new Error("idempotency mismatch was accepted");
  ...
  const expiredTarget = await repository.createHotel({
    ...
    value: { ...createInput, branchCode: "TEST-HOTEL-EXPIRED", name: "위아히어 만료키호텔" },
  });
  if (expiredTarget.status !== "CREATED") {
    throw new Error("expired target idempotency key was blocked behind cleanup backlog");
  }
  ...
  if (deniedScopedDetail) throw new Error("branch-scoped user accessed a hotel outside the grant");
  ...
  `;
  if (detailDeniedAudit[0]?.count !== 1) throw new Error("out-of-scope hotel detail denial was not audited");

  const actor = { companyId, sessionId, userId: actorUserId, userType: "INTERNAL_STAFF" as const };
  const listed = await repository.listHotels(actor);
  if (listed.status !== "OK"
    || !listed.capabilities.canCreate
    || !listed.hotels.some((hotel) => hotel.id === branchId && hotel.roadAddress === createInput.roadAddress)) {
    throw new Error("created hotel was not read back from PostgreSQL");
  }
  const normalizedPage = await repository.listHotels(actor, { page: 999, pageSize: 1 });
  if (normalizedPage.status !== "OK"
    ...
    throw new Error("out-of-range hotel page was not normalized to the final page");
  }
  if (await repository.getHotel(
    { companyId: otherCompanyId, sessionId, userId: otherUserId, userType: "INTERNAL_STAFF" },
    branchId,
  )) {
  ...
  const duplicate = await repository.createHotel({
    ...
    value: { ...createInput, branchCode: "TEST-HOTEL-03", name: "  위아히어 강남호텔  " },
  });
  if (duplicate.status !== "DUPLICATE") throw new Error("active hotel name duplicate was accepted");
  ...
  `;
  if (duplicateResidue[0]?.count !== 0) throw new Error("duplicate hotel left transaction residue");
  ...
  const concurrentActor = { companyId, sessionId, userId: actorUserId, userType: "INTERNAL_STAFF" as const };
  const concurrentResults = await Promise.all([
    repository.createHotel({
      ...ids("000000000008"), actor: concurrentActor,
      branchId: "51000000-0000-4000-8000-000000000008",
      idempotencyKey: "hotel-create-concurrent", operationPath: "/api/hotels",
      requestHash: "create-request-hash-concurrent", value: concurrentValue,
    }),
    repository.createHotel({
      ...ids("000000000009"), actor: concurrentActor,
  ...
  if (concurrentStatuses.join(",") !== "CREATED,REPLAYED") {
    throw new Error(`concurrent idempotency was not serialized: ${concurrentStatuses.join(",")}`);
  }
  const concurrentCounts = await testSql<{ audits: number; hotels: number; records: number }[]>`
    select
  ...
  `;
  if (concurrentCounts[0]?.hotels !== 1 || concurrentCounts[0].audits !== 1 || concurrentCounts[0].records !== 1) {
    throw new Error("concurrent idempotency persisted more than one business result");
  }

  const mismatchRaceResults = await Promise.all([
    repository.createHotel({
      ...ids("000000000020"), actor: concurrentActor,
      branchId: "51000000-0000-4000-8000-000000000020",
      idempotencyKey: "hotel-create-mismatch-race", operationPath: "/api/hotels",
      requestHash: "mismatch-race-a",
      value: { ...createInput, branchCode: "MISMATCH-RACE-A", name: "멱등 충돌 A" },
    }),
    repository.createHotel({
      ...ids("000000000021"), actor: concurrentActor,
      branchId: "51000000-0000-4000-8000-000000000021",
      idempotencyKey: "hotel-create-mismatch-race", operationPath: "/api/hotels",
      requestHash: "mismatch-race-b",
      value: { ...createInput, branchCode: "MISMATCH-RACE-B", name: "멱등 충돌 B" },
    }),
  ]);
  const mismatchRaceStatuses = mismatchRaceResults.map((result) => result.status).sort();
  if (mismatchRaceStatuses.join(",") !== "CREATED,IDEMPOTENCY_CONFLICT") {
    throw new Error(`concurrent idempotency mismatch was not serialized: ${mismatchRaceStatuses.join(",")}`);
  }
  const mismatchRaceCounts = await testSql<{ audits: number; hotels: number; records: number }[]>`
    select
  ...
  `;
  if (mismatchRaceCounts[0]?.hotels !== 1 || mismatchRaceCounts[0].audits !== 1 || mismatchRaceCounts[0].records !== 1) {
    throw new Error("concurrent idempotency mismatch left more than one business result");
  }

  const sameNameRaceResults = await Promise.all([
    repository.createHotel({
      ...ids("000000000022"), actor: concurrentActor,
      branchId: "51000000-0000-4000-8000-000000000022",
      idempotencyKey: "hotel-create-name-race-a", operationPath: "/api/hotels",
      requestHash: "name-race-a",
      value: { ...createInput, branchCode: "NAME-RACE-A", name: "동일 활성 호텔명" },
    }),
    repository.createHotel({
      ...ids("000000000023"), actor: concurrentActor,
      branchId: "51000000-0000-4000-8000-000000000023",
      idempotencyKey: "hotel-create-name-race-b", operationPath: "/api/hotels",
      requestHash: "name-race-b",
      value: { ...createInput, branchCode: "NAME-RACE-B", name: " 동일 활성 호텔명 " },
    }),
  ]);
  const sameNameRaceStatuses = sameNameRaceResults.map((result) => result.status).sort();
  if (sameNameRaceStatuses.join(",") !== "CREATED,DUPLICATE") {
    throw new Error(`concurrent active hotel names were not constrained: ${sameNameRaceStatuses.join(",")}`);
  }
  const sameNameRaceCounts = await testSql<{ audits: number; hotels: number; records: number }[]>`
    select
      (select count(*)::int from branches where company_id = ${companyId}
  ...
  `;
  if (sameNameRaceCounts[0]?.hotels !== 1 || sameNameRaceCounts[0].audits !== 1 || sameNameRaceCounts[0].records !== 1) {
    throw new Error("concurrent duplicate name failure left transaction residue");
  }
  ...
  try {
    await repository.createHotel({
      ...ids("000000000030"), actor: concurrentActor,
      branchId: "51000000-0000-4000-8000-000000000030",
      idempotencyKey: "hotel-create-profile-fail", operationPath: "/api/hotels",
      requestHash: "profile-fail",
      value: { ...createInput, branchCode: "PROFILE-FAIL", name: "프로필 실패 호텔" },
    });
  } catch {
    profileFailureRolledBack = true;
  } finally {
    await testSql.unsafe("drop trigger hotel_test_fail_profile on hotel_profiles; drop function hotel_test_fail_profile();");
  }
  if (!profileFailureRolledBack) throw new Error("profile failpoint did not fail hotel creation");
  ...
  try {
    await repository.createHotel({
      ...ids("000000000031"), actor: concurrentActor,
      branchId: "51000000-0000-4000-8000-000000000031",
      idempotencyKey: "hotel-create-audit-fail", operationPath: "/api/hotels",
      requestHash: "audit-fail",
      value: { ...createInput, branchCode: "AUDIT-FAIL", name: "감사 실패 호텔" },
    });
  } catch {
    auditFailureRolledBack = true;
  } finally {
    await testSql.unsafe("drop trigger hotel_test_fail_audit on audit_events; drop function hotel_test_fail_audit();");
  }
  if (!auditFailureRolledBack) throw new Error("audit failpoint did not fail hotel creation");
  ...
  try {
    await repository.createHotel({
      ...ids("000000000032"), actor: concurrentActor,
      branchId: "51000000-0000-4000-8000-000000000032",
      idempotencyKey: "hotel-create-completion-fail", operationPath: "/api/hotels",
      requestHash: "completion-fail",
      value: { ...createInput, branchCode: "COMPLETION-FAIL", name: "멱등 완료 실패 호텔" },
    });
  } catch {
    completionFailureRolledBack = true;
  } finally {
    await testSql.unsafe("drop trigger hotel_test_fail_completion on idempotency_records; drop function hotel_test_fail_completion();");
  }
  if (!completionFailureRolledBack) throw new Error("idempotency completion failpoint did not fail hotel creation");
  ...
  `;
  if (failpointResidue[0]?.count !== 0) throw new Error("hotel create failpoints left transaction residue");

  const auditRows = await testSql<{ event_code: string; has_sensitive_fields: boolean }[]>`
    select event_code,
  ...
  if (auditRows.length !== 1 || auditRows[0]?.has_sensitive_fields) {
    throw new Error("hotel create audit was missing or contained sensitive contact/address fields");
  }
  ...
  if (forbiddenList.status !== "FORBIDDEN") {
    throw new Error("missing HOTEL_MANAGE was disguised as an empty hotel list");
  }
  ...
  if (deniedList.status !== "FORBIDDEN") throw new Error("company HOTEL_MANAGE deny did not block the list");
  ...
  `;
  if (listDeniedAudit[0]?.count !== 1) throw new Error("hotel list denial was not audited");
  ...
  if (forbidden.status !== "FORBIDDEN") throw new Error("hotel create without HOTEL_MANAGE was accepted");
  ...
  `;
  if (deniedAudit[0]?.count !== 1) throw new Error("valid permission denial was not audited safely");
