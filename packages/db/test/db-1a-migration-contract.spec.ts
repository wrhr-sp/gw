...
const migration6 = readFileSync(
  new URL("../migrations/0006_account_administration.sql", import.meta.url),
  "utf8",
);
const migration7 = readFileSync(
  new URL(
    "../migrations/0007_api_tenant_authority_expand.sql",
    import.meta.url,
  ),
  "utf8",
);
const migration8 = readFileSync(
  new URL(
    "../migrations/0008_remove_legacy_company_id_fallback.sql",
    import.meta.url,
  ),
  "utf8",
);
...
describe("DB-1A durable account migration contracts", () => {
  it("models durable dispatch states, terminal slot release, and post-expiry terminal evidence", () => {
    ...
    for (const state of [
      ...
      expect(migration6).toContain(`'${state}'`);
    expect(migration6).toMatch(
      /account_provisioning_attempts_active_user_unique_idx[\s\S]+where status in \('RESERVED_NOT_DISPATCHED', 'DISPATCHED', 'PROVIDER_CONFIRMED', 'RECOVERY_REQUIRED', 'COMPENSATION_REQUIRED'\)/i,
    );
    expect(migration6).toMatch(
      /initial_password_change_attempts_active_user_unique_idx[\s\S]+where status in \('RESERVED_NOT_DISPATCHED', 'DISPATCHED', 'PROVIDER_UPDATED', 'RECOVERY_REQUIRED'\)/i,
    );
    for (const column of [
      "dispatched_at",
    ...
    expect(migration6).not.toMatch(
      /delete\s+from\s+(account_provisioning_attempts|initial_password_change_attempts)/i,
    );
  });

  it("binds password attempts to the exact tenant session principal", () => {
    expect(migration6).toMatch(
      /alter table auth_sessions[\s\S]+unique \(company_id, id, user_id\)/i,
    );
    expect(migration6).toMatch(
      /foreign key \(company_id, session_id, user_id\) references auth_sessions\(company_id, id, user_id\)/i,
    );
  });
...
describe("DB-1A auth v2 and tenant authority contracts", () => {
  it("adds v2 auth functions without replacing or dropping v1", () => {
    for (const fn of [
      "auth_create_session",
      "auth_resolve_principal",
      "auth_revoke_session",
    ]) {
      expect(migration7).toMatch(
        new RegExp(
          `create\\s+(?:or\\s+replace\\s+)?function\\s+public\\.${fn}_v2\\b`,
          "i",
        ),
      );
      expect(migration7).not.toMatch(
        new RegExp(
          `(?:create\\s+(?:or\\s+replace\\s+)?|drop\\s+)function\\s+(?:public\\.)?${fn}\\s*\\(`,
          "i",
        ),
      );
    }
    ...
    expect(migration7).toContain(
      "app_user.status in ('ACTIVE', 'PENDING_SETUP')",
    );
  ...
  it("provides an API-tenant-bound definer for user-wide session revocation", () => {
    expect(migration7).toMatch(
      /create function public\.auth_revoke_user_sessions_v1\(\s*p_company_id uuid,\s*p_user_id uuid,\s*p_reason text/iu,
    );
    expect(migration7).toMatch(
      /auth_revoke_user_sessions_v1[\s\S]+security definer[\s\S]+set search_path = pg_catalog/iu,
    );
    expect(migration7).toContain(
      "p_company_id is distinct from public.api_current_company_id()",
    );
    expect(migration7).toContain(
      "p_reason not in ('ACCOUNT_DEACTIVATED', 'INITIAL_PASSWORD_CHANGED')",
    );
    expect(migration7).toMatch(
      /alter function public\.auth_revoke_user_sessions_v1\(uuid, uuid, text\)\s+owner to werehere_auth_session_definer/iu,
    );
    expect(migration7).toMatch(
      /revoke all on function public\.auth_revoke_user_sessions_v1\(uuid, uuid, text\) from public/iu,
    );
  });
  ...
  it("uses isolated hardened owners, public qualification, and no residual membership", () => {
    ...
    expect(migration7).toMatch(
      /create role werehere_tenant_authority_definer\s+nologin noinherit nosuperuser/i,
    );
    expect(migration7).toMatch(
      /revoke werehere_auth_session_definer from %I granted by %I/i,
    );
    expect(migration7).toMatch(
      /revoke werehere_tenant_authority_definer from %I granted by %I/i,
    );
    ...
    expect(migration7).toMatch(
      /revoke all on function public\.runtime_has_capability\(text\) from public/i,
    );
  ...
  it("expands named policies with capability CASE branches and legacy fallback without drop-all", () => {
    ...
    expect(migration7).toContain(
      "not public.runtime_has_capability(''API_RUNTIME'')",
    );
    expect(migration7).toContain(
      "not public.runtime_has_capability(''RECONCILER'')",
    );
    expect(migration7).toContain(
  ...
  it("removes app.company_id authority from every tenant policy and fails unknown runtimes closed", () => {
    expect(migration8).not.toContain("app.company_id");
    expect(migration8).toContain("revoke usage on schema public from public");
    ...
    expect(migration8.match(/else false/g)).toHaveLength(2);
    expect(migration8.match(/_company_isolation'/g)).toHaveLength(19);
    expect(migration8).toContain(
      "alter table public.%I force row level security",
    );
  });
