...
try {
  ...
  `;
  if (!companies[0] || !companies[1]) throw new Error("RLS integration requires two active companies");
  ...
  `;
  if (unscoped[0]?.count !== 0) throw new Error("RLS exposed hotel rows without app.company_id");

  await sql.begin(async (transaction) => {
    await transaction`select set_config('app.company_id', ${primaryCompanyId}, true)`;
    const visibleCompanies = await transaction<{ company_id: string }[]>`
      select distinct company_id from branches where branch_type = 'HOTEL'
    `;
    if (visibleCompanies.some((row) => row.company_id !== primaryCompanyId)) {
      throw new Error("RLS exposed another company's branch");
    }
    const crossProfiles = await transaction<{ count: number }[]>`
      select count(*)::int as count from hotel_profiles where company_id = ${otherCompanyId}
    `;
    if (crossProfiles[0]?.count !== 0) throw new Error("RLS exposed another company's hotel profile");
  });
  ...
  try {
    await sql.begin(async (transaction) => {
      ...
      await transaction`
        insert into branches (id, company_id, branch_code, name, branch_type, status)
        values (
          '52000000-0000-4000-8000-000000000098', ${otherCompanyId},
          'RLS-BLOCK-98', 'RLS 차단 대상', 'HOTEL', 'ACTIVE'
        )
      `;
    });
  } catch (error) {
    blocked = Boolean(error && typeof error === 'object' && 'code' in error && error.code === '42501');
  }
  if (!blocked) throw new Error("RLS allowed a cross-company branch insert");

  console.log("HOTEL_RLS_INTEGRATION_OK");
} finally {
  await sql.unsafe("reset role").catch(() => undefined);
  await sql`delete from hotel_profiles where branch_id = ${crossBranchId}`.catch(() => undefined);
  await sql`delete from branches where id = ${crossBranchId}`.catch(() => undefined);
