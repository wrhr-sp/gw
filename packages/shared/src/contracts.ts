...
export const mailMessageSchema = z.object({
  ...
  senderName: z.string(),
  senderMailAccountId: z.string().nullable().optional(),
  senderMailAliasId: z.string().nullable().optional(),
...
export const adminUserCreateRequestSchema = z
  .object({
    ...
    addressPostalCode: z.string().trim().min(1).max(20).optional(),
    addressBase: z.string().trim().min(1).max(300).optional(),
    addressDetail: z.string().trim().min(1).max(300).optional(),
    employmentCategory: z.string().trim().min(1).max(50).optional(),
    departmentName: z.string().trim().min(1).max(100),
    branchName: z.string().trim().min(1).max(100),
    positionName: z.string().trim().max(100).optional(),
    accountType: adminAccountTypeSchema,
    roleCode: roleCodeSchema,
    status: adminAccountStatusSchema.default("invited"),
    reason: z.string().trim().min(1).max(500),
    mustChangePassword: z.boolean().optional(),
