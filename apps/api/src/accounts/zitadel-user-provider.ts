...
const verificationSessionReadSchema = z.object({
  session: z.object({
    id: z.string().min(1).max(200),
    factors: z.object({
      user: z.object({
      ...
      password: z.object({ verifiedAt: z.iso.datetime({ offset: true }) }).passthrough(),
    }).passthrough(),
  }).passthrough(),
}).passthrough();
const getResponseSchema = z.object({
  user: z.object({
    ...
    details: z.object({ resourceOwner: z.string().min(1).max(200) }).passthrough(),
...
export function createZitadelUserProvider(options: ZitadelUserProviderOptions): UserProvisioningProvider {
  ...
  async function request(
    ...
    if (response.status === 409) throw new AccountServiceError("ACCOUNT_DUPLICATE", 409, false);
    if (response.status === 401 || response.status === 403) {
      throw new AccountServiceError("EXTERNAL_AUTH_NOT_CONFIGURED", 503, false);
    }
    if (!response.ok) throw new AccountServiceError("EXTERNAL_AUTH_UNAVAILABLE", 503, response.status >= 500);
    return response;
  }
  ...
  return {
    ...
    async humanUserExists(subject) {
      const response = await request(`/v2/users/${encodeURIComponent(subject)}`, { method: "GET" }, [404]);
    ...
    async verifyPassword(input) {
      const createResponse = await request("/v2/sessions", {
        method: "POST",
        body: JSON.stringify({
          checks: {
      ...
      }, [400, 404], verificationToken);
      if (createResponse.status === 400 || createResponse.status === 404) return false;
      let createBody: unknown;
      try { createBody = await createResponse.json(); } catch {
        throw new AccountServiceError("EXTERNAL_AUTH_UNAVAILABLE", 503, true);
      }
      const created = verificationSessionSchema.safeParse(createBody);
      if (!created.success) throw new AccountServiceError("EXTERNAL_AUTH_UNAVAILABLE", 503, true);
      ...
      try {
        ...
        try { sessionBody = await sessionResponse.json(); } catch {
          throw new AccountServiceError("EXTERNAL_AUTH_UNAVAILABLE", 503, true);
        }
        ...
        try {
          await request(`/v2/sessions/${encodeURIComponent(created.data.sessionId)}`, {
            method: "DELETE",
    ...
    async deactivateHumanUser(subject) {
      try {
        await request(`/v2/users/${encodeURIComponent(subject)}/deactivate`, {
          method: "POST",
          body: "{}",
        }, [404]);
      } catch (error) {
        if (!(error instanceof AccountServiceError) || error.code !== "ACCOUNT_DUPLICATE") throw error;
      }
    },
    ...
    async setPassword(subject, newPassword) {
      await request(`/v2/users/${encodeURIComponent(subject)}/password`, {
        method: "POST",
        body: JSON.stringify({ newPassword: { password: newPassword, changeRequired: false } }),
      });
    },
  };
}
