...
import { createAccountRequestSchema, initialPasswordRequestSchema } from "@werehere/contracts";
...
export interface UserProvisioningProvider {
  ...
  deactivateHumanUser(subject: string): Promise<void>;
...
export interface AccountService {
  ...
  getCapabilities(principal: AuthenticatedPrincipal): Promise<{ permissions: AccountPermission[] }>;
  deactivateAccount(
    principal: AuthenticatedPrincipal,
    userId: string,
...
export function createAccountService(input: {
  ...
  return {
    close() { return input.repository.close?.() ?? Promise.resolve(); },

    async createAccount(principal, value, idempotencyKey) {
      const parsed = createAccountRequestSchema.safeParse(value);
      if (!parsed.success) throw new AccountServiceError(
        "VALIDATION_ERROR", 400, false,
        parsed.error.issues.map((issue) => ({ field: String(issue.path[0] ?? "initialPassword"), message: issue.message })),
      );
      value = parsed.data;
      const accountId = crypto.randomUUID();
      const hotelIds = value.userType === "HOUSEKEEPING"
        ? value.hotelIds ?? []
        : value.hotelId ? [value.hotelId] : [];
      const nonSecretRequest = {
        displayName: value.displayName,
        loginName: value.loginName,
        email: value.email,
        userType: value.userType,
        ...(value.userType === "HOUSEKEEPING" ? { hotelIds } : { hotelId: hotelIds[0] }),
      ...
      if (reserved.status === "FORBIDDEN") throw new AccountServiceError("FORBIDDEN", 403, false);
      if (reserved.status === "HOTEL_NOT_FOUND") throw new AccountServiceError("RESOURCE_NOT_FOUND", 404, false);
      if (reserved.status === "COMPENSATED") throw new AccountServiceError("ACCOUNT_DUPLICATE", 409, false);
      if (reserved.status === "COMPENSATION_REQUIRED") throw new AccountServiceError("COMPENSATION_REQUIRED", 503, false);
      if (reserved.status === "RECOVERY_REQUIRED") throw new AccountServiceError("EXTERNAL_AUTH_UNAVAILABLE", 503, true);
      if (reserved.status === "OPERATOR_REQUIRED" || reserved.status === "DEAD_LETTER") {
        throw new AccountServiceError("INTERNAL_ERROR", 500, false);
      }
      if (reserved.status === "IDEMPOTENCY_CONFLICT") throw new AccountServiceError("IDEMPOTENCY_CONFLICT", 409, false);
      if (reserved.status === "IN_PROGRESS") throw new AccountServiceError("IDEMPOTENCY_CONFLICT", 409, true);
      if (reserved.status === "REPLAYED") {
        throw new AccountServiceError("IDEMPOTENCY_CONFLICT", 409, false);
      }
      if (!("accountId" in reserved)) throw new AccountServiceError("INTERNAL_ERROR", 500, true);

      const targetId = reserved.accountId;
      const leaseVersion = reserved.leaseVersion;
      let subject = reserved.status === "PROVIDER_CONFIRMED" ? reserved.subject : null;
      let providerCreated = subject !== null;
      let compensationAllowed = false;
      try {
        if (subject && !await input.provider.verifyPassword({
        ...
        if (!("account" in completed)) throw new AccountServiceError("INTERNAL_ERROR", 500, true);
        const outcome = await input.repository.getCreateOutcome({
          accountId: targetId,
          companyId: principal.companyId,
          sessionId: principal.sessionId,
        });
        if (outcome.status !== "COMPLETED") throw new AccountServiceError("INTERNAL_ERROR", 500, true);
    ...
    async deactivateAccount(principal, userId, value, idempotencyKey = crypto.randomUUID()) {
      if (principal.userId === userId) {
        throw new AccountServiceError("ACCOUNT_SELF_DEACTIVATION_FORBIDDEN", 409, false);
      }
      const result = await input.repository.deactivateAccount({
        actor: actor(principal),
        auditEventId: crypto.randomUUID(),
        idempotencyKey,
        requestHash: await sha256(JSON.stringify({ userId, version: value.version, reason: value.reason })),
        targetUserId: userId,
        traceId: crypto.randomUUID(),
        value,
      });
      if (result.status === "FORBIDDEN") throw new AccountServiceError("FORBIDDEN", 403, false);
      if (result.status === "NOT_FOUND") throw new AccountServiceError("ACCOUNT_NOT_FOUND", 404, false);
      if (result.status === "VERSION_CONFLICT") throw new AccountServiceError("ACCOUNT_VERSION_CONFLICT", 409, false);
    ...
    async changeInitialPassword(principal, value, idempotencyKey) {
      ...
      if (!parsed.success) throw new AccountServiceError(
        "VALIDATION_ERROR", 400, false,
        parsed.error.issues.map((issue) => ({ field: String(issue.path[0] ?? "newPassword"), message: issue.message })),
      );
      value = parsed.data;
      ...
      if (reserved.status === "NOT_PENDING") throw new AccountServiceError("INVALID_STATE_TRANSITION", 409, false);
      if (reserved.status === "IN_PROGRESS") throw new AccountServiceError("IDEMPOTENCY_CONFLICT", 409, true);
      if (reserved.status === "RECOVERY_REQUIRED") throw new AccountServiceError("PASSWORD_RECOVERY_REQUIRED", 503, true);
      if (reserved.status === "REPLAYED") throw new AccountServiceError("IDEMPOTENCY_CONFLICT", 409, false);
      let operationKey = idempotencyKey;
      if (reserved.status === "RECOVERY_CONFIRMABLE" || reserved.status === "PROVIDER_UPDATED") {
        ...
        if (!verified) throw new AccountServiceError("PASSWORD_RECOVERY_REQUIRED", 409, false);
        if (reserved.status === "RECOVERY_CONFIRMABLE") {
          const confirmed = await input.repository.confirmInitialPasswordProviderState({
            companyId: principal.companyId,
            sessionId: principal.sessionId,
            userId: principal.userId,
      ...
      if (reserved.status === "RESERVED_NOT_DISPATCHED") {
        ...
        const marked = await input.repository.markInitialPasswordProviderUpdated({
          companyId: principal.companyId,
          sessionId: principal.sessionId,
          userId: principal.userId,
          idempotencyKey: operationKey,
