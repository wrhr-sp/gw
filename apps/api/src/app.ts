import {
  ...
  calendarOAuthStartRequestSchema,
...
import {
  createCalendarConnectionServiceFromBindings,
...
} from "./calendar-projections/service";
...
const CALENDAR_OAUTH_BROWSER_COOKIE_NAME = "__Host-hotel_calendar_oauth";
...
export function createApp(options: CreateAppOptions = {}) {
  ...
  function getCalendarConnectionService(bindings: Bindings | undefined) {
    return (
      options.calendarConnectionService ??
      createCalendarConnectionServiceFromBindings(bindings)
    );
  }
  ...
  async function withCalendarConnectionService<T>(
    bindings: Bindings | undefined,
    operation: (service: CalendarConnectionService) => Promise<T>,
  ): Promise<T> {
    const service = getCalendarConnectionService(bindings);
    try {
      return await operation(service);
    } finally {
      if (!options.calendarConnectionService) await service.close?.();
    }
  }
  ...
  hotelApp.get("/api/admin/calendar-connections", async (context) => {
    context.header("Cache-Control", "private, no-store");
    try {
      const principal = await requestPrincipal(context);
      if (!principal)
      ...
      const data = await withCalendarConnectionService(context.env, (service) =>
        service.status(roomMutationPrincipal(context, principal)),
      );
      return context.json(
        calendarConnectionStatusResponseSchema.parse({
          ok: true,
          data,
          error: null,
        }),
  ...
  hotelApp.post(
    ...
    async (context) => {
      ...
      try {
        ...
        const body = calendarOAuthStartRequestSchema.safeParse(
          await context.req.json().catch(() => undefined),
        );
        if (!body.success)
          return validationFailure(context, zodFieldErrors(body.error.issues));
        const result = await withCalendarConnectionService(
          context.env,
          (service) =>
            service.oauthStart(
              roomMutationPrincipal(context, principal),
        ...
        setCookie(
          context,
          CALENDAR_OAUTH_BROWSER_COOKIE_NAME,
          result.browserBinding,
          { ...OAUTH_BROWSER_COOKIE_OPTIONS, sameSite: "Lax" },
        );
        return context.json(
          calendarOAuthStartResponseSchema.parse({
            ok: true,
  ...
  hotelApp.get(
    ...
    async (context) => {
      ...
      context.header("Referrer-Policy", "no-referrer");
      const clearBinding = () =>
        setCookie(context, CALENDAR_OAUTH_BROWSER_COOKIE_NAME, "", {
          ...OAUTH_BROWSER_COOKIE_OPTIONS,
          maxAge: 0,
        });
      try {
        ...
        const binding = readUniqueCookie(
          context,
          CALENDAR_OAUTH_BROWSER_COOKIE_NAME,
        );
        const malformed =
          ...
          !binding;
        if (malformed) {
          if (states.length === 1 && states[0] && binding) {
            const rejected = await withCalendarConnectionService(
              context.env,
              (service) =>
                service.oauthRejectMalformed({
                  state: states[0]!,
                  browserBinding: binding,
                }),
            );
            clearBinding();
            return context.redirect(rejected.returnPath, 303);
          }
        ...
        const result = await withCalendarConnectionService(
          ...
          (service) =>
            service.oauthCallback({
              state: states[0]!,
              code: codes[0] ?? null,
              providerError: errors[0] ?? null,
              browserBinding: binding,
            }),
        );
        clearBinding();
        return context.redirect(result.returnPath, 303);
      } catch (error) {
        clearBinding();
        if (error instanceof CalendarConnectionServiceError)
          return context.redirect(error.returnPath ?? "/admin/calendar", 303);
        return context.redirect("/admin/calendar", 303);
      }
    },
  );

  const connectionResourceCommand = async (
    context: Context<{ Bindings: Bindings }>,
    ...
    try {
      ...
      const connectionId = HOTEL_ID_SCHEMA.safeParse(
        context.req.param("connectionId"),
      );
      const candidateId = HOTEL_ID_SCHEMA.safeParse(
        context.req.param("candidateId"),
      );
      const payload = await context.req.json().catch(() => undefined);
      if (
        !connectionId.success ||
        (action !== "DISCONNECT" && !candidateId.success)
      )
        return mutationFailure(context, "NOT_FOUND");
      const actor = roomMutationPrincipal(context, principal);
      const execute = async () => {
        if (action === "DISCONNECT") {
          const body =
            calendarConnectionDisconnectRequestSchema.safeParse(payload);
          if (!body.success)
            return validationFailure(
              context,
              zodFieldErrors(body.error.issues),
            );
          return withCalendarConnectionService(context.env, (service) =>
        ...
        const body =
          calendarCredentialCandidateCommandRequestSchema.safeParse(payload);
        if (!body.success)
          return validationFailure(context, zodFieldErrors(body.error.issues));
        return withCalendarConnectionService(context.env, (service) =>
          service.connectionCommand(
      ...
      return context.json(
        calendarConnectionCommandResponseSchema.parse({
          ok: true,
          data,
          error: null,
        }),
  ...
  hotelApp.post(
    "/api/admin/calendar-connections/:connectionId/credential-candidates/:candidateId/promote",
    (context) => connectionResourceCommand(context, "PROMOTE_CANDIDATE"),
  );
  hotelApp.post(
    "/api/admin/calendar-connections/:connectionId/credential-candidates/:candidateId/confirm-switch",
    (context) => connectionResourceCommand(context, "CONFIRM_ACCOUNT_CHANGE"),
  );
  hotelApp.post(
    "/api/admin/calendar-connections/:connectionId/disconnect",
    (context) => connectionResourceCommand(context, "DISCONNECT"),
  );

  const hotelCalendarResourceCommand = async (
    context: Context<{ Bindings: Bindings }>,
    ...
    try {
      ...
      const payload = await context.req.json().catch(() => undefined);
      if (!connectionId.success) return mutationFailure(context, "NOT_FOUND");
      const actor = roomMutationPrincipal(context, principal);
      const execute = async () => {
        if (action === "CREATE") {
          const body = calendarHotelCreateRequestSchema.safeParse(payload);
          if (!body.success)
            return validationFailure(
              context,
              zodFieldErrors(body.error.issues),
          ...
          return withCalendarConnectionService(context.env, (service) =>
            service.hotelLinkCommand(
              actor,
              connectionId.data,
              body.data.branchId,
              {
                action,
                expectedConnectionVersion: body.data.expectedConnectionVersion,
                expectedVersion: 0,
                reason: "호텔 Calendar 생성",
              },
              key,
            ),
          );
        ...
        const hotelId = HOTEL_ID_SCHEMA.safeParse(context.req.param("hotelId"));
        const body = calendarHotelDisconnectRequestSchema.safeParse(payload);
        if (!hotelId.success) return mutationFailure(context, "NOT_FOUND");
        if (!body.success)
          return validationFailure(context, zodFieldErrors(body.error.issues));
        return withCalendarConnectionService(context.env, (service) =>
          service.hotelLinkCommand(
            ...
            hotelId.data,
            {
              action,
              expectedConnectionVersion: body.data.expectedConnectionVersion,
              expectedVersion: body.data.expectedLinkVersion,
  ...
  hotelApp.post(
    "/api/admin/calendar-connections/:connectionId/hotel-calendars",
    (context) => hotelCalendarResourceCommand(context, "CREATE"),
  );
  hotelApp.post(
    "/api/admin/calendar-connections/:connectionId/hotel-calendars/:hotelId/disconnect",
    (context) => hotelCalendarResourceCommand(context, "DISCONNECT"),
  );

  hotelApp.post(
    "/api/admin/calendar-connections/hotels/:hotelId/failures/:failureId/retry",
    async (context) => {
      context.header("Cache-Control", "no-store");
      try {
        const principal = await requestPrincipal(context);
        ...
        const hotelId = HOTEL_ID_SCHEMA.safeParse(context.req.param("hotelId"));
        const failureId = HOTEL_ID_SCHEMA.safeParse(
          context.req.param("failureId"),
        );
        const body = calendarFailureRetryRequestSchema.safeParse(
          await context.req.json().catch(() => undefined),
        );
        if (!hotelId.success || !failureId.success)
          return mutationFailure(context, "NOT_FOUND");
        if (!body.success)
          return validationFailure(context, zodFieldErrors(body.error.issues));
        const data = await withCalendarConnectionService(
          context.env,
          (service) =>
            service.failureRetry(
              roomMutationPrincipal(context, principal),
              hotelId.data,
              failureId.data,
              body.data,
