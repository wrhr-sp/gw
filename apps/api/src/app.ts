...
import {
  ...
  organizationCodePolicyUpdateRequestSchema,
...
const permissionCatalog: Permission[] = [
  ...
  { code: "attendance.read", description: "본인 근태 기록 조회와 출퇴근 처리 흐름을 사용한다." },
  { code: "attendance.manage", description: "근태 정정 검토와 관리자 조회 확장을 위한 권한 골격이다." },
  { code: "leave.request", description: "휴가 유형/잔여 조회와 휴가 신청 흐름을 사용한다." },
  { code: "leave.approve", description: "휴가 승인/반려 흐름을 처리한다." },
  ...
  { code: "approval.form.manage", description: "전자결재 양식 정의를 관리한다." },
  { code: "approval.line.manage", description: "전자결재 결재선 정의를 관리한다." },
  { code: "approval.document.read", description: "전자결재 문서함과 상세 조회를 사용한다." },
  { code: "approval.document.write", description: "전자결재 기안 흐름을 사용한다." },
  { code: "approval.document.approve", description: "전자결재 승인함과 승인/반려 흐름을 처리한다." },
  { code: "board.notice.read", description: "사내 공지형 게시판 목록을 조회한다." },
  { code: "board.manage", description: "게시판 생성과 관리 흐름을 처리한다." },
  { code: "board.post.write", description: "게시글 작성 흐름을 사용한다." },
  { code: "board.comment.write", description: "게시글 댓글 작성 흐름을 사용한다." },
  { code: "document.space.read", description: "문서함 목록과 접근 가능한 공간을 조회한다." },
  { code: "document.space.manage", description: "문서함 생성과 관리 흐름을 처리한다." },
  { code: "document.file.read", description: "문서 메타데이터 목록과 다운로드 후보를 조회한다." },
  { code: "document.file.write", description: "문서 업로드 메타데이터 생성을 처리한다." },
  ...
  { code: "work_item.deadline.read", description: "공통 업무 마감 캘린더와 임박 상태를 조회한다." },
  { code: "work_item.audit.read", description: "공통 업무 감사 로그를 읽기 전용으로 조회한다." },
...
const payrollRoleGuidance = {
  ...
  restrictedActions: [
    "세액 계산, 4대보험 확정, 외부 신고 연동은 별도 승인 게이트에서 다룬다.",
...
app.get(appRoutes.admin.organizationInfo, async (context) => {
  ...
  if (!masters) return jsonDatabaseRequired(context, "조직정보 조회");
...
app.post("/api/admin/organization-info/:kind", async (context) => {
  const authResult = requireAdminRole(context);
  if ("response" in authResult) return authResult.response;
...
app.patch(appRoutes.admin.organizationInfoItem(":kind", ":id"), async (context) => {
  const authResult = requireAdminRole(context);
  if ("response" in authResult) return authResult.response;
  if (!hasPermission(authResult.auth.user, "employee.write")) return jsonError(context, "FORBIDDEN", "필요한 권한이 없습니다.", 403);
  const kindParsed = employeeOrganizationMasterKindSchema.safeParse(context.req.param("kind"));
...
app.post(appRoutes.admin.organizationInfoItemStatus(":kind", ":id"), async (context) => {
  const authResult = requireAdminRole(context);
  if ("response" in authResult) return authResult.response;
  if (!hasPermission(authResult.auth.user, "employee.write")) return jsonError(context, "FORBIDDEN", "필요한 권한이 없습니다.", 403);
  const kindParsed = employeeOrganizationMasterKindSchema.safeParse(context.req.param("kind"));
...
app.get(appRoutes.admin.organizationCodePolicies, async (context) => {
  const authResult = requireAdminRole(context);
  if ("response" in authResult) return authResult.response;
  if (!hasPermission(authResult.auth.user, "employee.write")) return jsonError(context, "FORBIDDEN", "필요한 권한이 없습니다.", 403);
  const items = await listOperationalOrganizationCodePolicies(context.env, authResult.auth.user.companyId);
  if (!items) return jsonDatabaseRequired(context, "조직정보 코드정보 정책 조회");
  return jsonSuccess(context, organizationCodePoliciesResponseSchema, { ok: true, data: { items, audit: { candidate: true, action: "admin.organization_code_policies.list.viewed" } }, error: null }, 200);
});

app.patch(appRoutes.admin.organizationCodePolicy(":kind"), async (context) => {
  const authResult = requireAdminRole(context);
  if ("response" in authResult) return authResult.response;
  if (!hasPermission(authResult.auth.user, "employee.write")) return jsonError(context, "FORBIDDEN", "필요한 권한이 없습니다.", 403);
  const kindParsed = organizationCodePolicyKindSchema.safeParse(context.req.param("kind"));
  if (!kindParsed.success) return jsonError(context, "VALIDATION_ERROR", "코드정보 정책 종류가 올바르지 않습니다.", 400);
  const parsed = organizationCodePolicyUpdateRequestSchema.safeParse(await context.req.json().catch(() => null));
  if (!parsed.success) return jsonError(context, "VALIDATION_ERROR", "코드정보 정책 요청 형식이 올바르지 않습니다.", 400, { issues: parsed.error.issues });
  const result = await upsertOperationalOrganizationCodePolicy(context.env, authResult.auth.user.companyId, authResult.auth.user.id, kindParsed.data, parsed.data);
  if (!result) return jsonDatabaseRequired(context, "조직정보 코드정보 정책 저장");
  return jsonSuccess(context, organizationCodePolicyMutationResponseSchema, { ok: true, data: { item: result.item, audit: { candidate: true, action: "admin.organization_code_policy.update" }, persistence: "operational-db", updatedAt: result.updatedAt }, error: null }, 200);
});
...
app.post(appRoutes.admin.userCreate, async (context) => {
  ...
  const body = isFormRequest
    ? await context.req.parseBody().then((form) => ({
        ...
        recognizedHireDate: form.recognizedHireDate,
        contactPhone: form.contactPhone,
        externalEmail: form.externalEmail,
        addressPostalCode: form.addressPostalCode,
        addressBase: form.addressBase,
...
app.patch(appRoutes.admin.userOrganization(":userId"), async (context) => {
  const authResult = requireAdminRole(context);
  if (authResult.response) {
    return authResult.response;
  }
  ...
  const body = await context.req.json().catch(() => null);
  const parsed = adminUserOrganizationUpdateRequestSchema.safeParse(body);
  if (!parsed.success) {
    return jsonError(context, "VALIDATION_ERROR", "사원 조직정보 저장 요청 형식이 올바르지 않습니다.", 400, { issues: parsed.error.issues });
  }

  const result = await updateOperationalAdminUserOrganization(
    context.env,
    authResult.auth.user.companyId,
    authResult.auth.user.id,
    targetUserId,
  ...
  if (!result) {
    return jsonDatabaseRequired(context, "사원 조직정보 저장");
  }
  ...
  return jsonSuccess(
    ...
    adminUserMutationResponseSchema,
    {
      ...
      data: {
        user: result.user,
        audit: { candidate: true, action: "admin.user.organization.update" },
        persistence: "operational-db",
        updatedAt: result.updatedAt,
      },
      error: null,
...
app.post(appRoutes.admin.userSecurity(":userId"), async (context) => {
  const authResult = requireAdminRole(context);
  if (authResult.response) {
    return authResult.response;
  }
  ...
  const body = await context.req.json().catch(() => null);
  const parsed = adminUserSecurityUpdateRequestSchema.safeParse(body);
  if (!parsed.success) {
    return jsonError(context, "VALIDATION_ERROR", "보안 설정 저장 요청 형식이 올바르지 않습니다.", 400, { issues: parsed.error.issues });
  }

  const result = await updateOperationalAdminUserSecurity(
    context.env,
    authResult.auth.user.companyId,
    authResult.auth.user.id,
    targetUserId,
  ...
  if (!result) {
    return jsonDatabaseRequired(context, "사원 보안 설정 저장");
  }
  ...
  return jsonSuccess(
    ...
    adminUserMutationResponseSchema,
    {
      ...
      data: {
        user: result.user,
        audit: { candidate: true, action: "admin.user.security.update" },
        persistence: "operational-db",
        updatedAt: result.updatedAt,
      },
      error: null,
...
app.get(ERP_VENDORS_ROUTE, async (context) => {
  ...
  await recordOperationalPrivacyAccessEvent(context.env, {
    ...
    metadata: { source: "erp-vendors-api", itemCount: result.items.length, externalProvider: "optional-external-erp" },
  });
...
app.get(ERP_EXPENSES_ROUTE, async (context) => {
  ...
  await recordOperationalPrivacyAccessEvent(context.env, {
    ...
    legalBasis: "부서업무포털 권한 기반 경리 업무 접근",
    metadata: { source: "erp-expenses-api", itemCount: result.items.length, externalProvider: "optional-external-erp" },
  });
...
app.get(ERP_EVIDENCE_ROUTE, async (context) => {
  ...
  await recordOperationalPrivacyAccessEvent(context.env, {
    ...
    metadata: { source: "erp-evidence-api", itemCount: result.items.length, externalProvider: "optional-external-erp" },
  });
...
app.get(ERP_BILLINGS_ROUTE, async (context) => {
  ...
  await recordOperationalPrivacyAccessEvent(context.env, {
    ...
    metadata: { source: "erp-billings-api", itemCount: result.items.length, externalProvider: "optional-external-erp" },
  });
...
app.get(ERP_PAYMENT_RECORDS_ROUTE, async (context) => {
  ...
  await recordOperationalPrivacyAccessEvent(context.env, {
    ...
    metadata: { source: "erp-payment-records-api", itemCount: result.items.length, externalProvider: "optional-external-erp" },
  });
...
app.get(ERP_ACCOUNTING_MAPPINGS_ROUTE, async (context) => {
  ...
  await recordOperationalPrivacyAccessEvent(context.env, {
    ...
    metadata: { source: "erp-accounting-mappings-api", itemCount: result.items.length, externalProvider: "optional-external-erp" },
  });
...
app.get(ERP_INTEGRATION_EVENTS_ROUTE, async (context) => {
  ...
  if (!result) return jsonDatabaseRequired(context, "ERP 외부 연동 로그 목록 조회");
  ...
  await recordOperationalPrivacyAccessEvent(context.env, {
    ...
    accessType: "read",
    purpose: "ERP 외부 연동 로그 목록 조회 감사 기록",
    legalBasis: "부서업무포털 권한 기반 경리 연동 로그 접근",
    metadata: { source: "erp-integration-events-api", itemCount: result.items.length, externalProvider: "optional-external-erp" },
  });
...
app.post(ERP_INTEGRATION_EVENTS_ROUTE, async (context) => {
  ...
  if (!created?.event) return jsonDatabaseRequired(context, "ERP 외부 연동 로그 생성");
...
app.get(ERP_INTEGRATION_EVENT_DETAIL_ROUTE, async (context) => {
  ...
  if (!result) return jsonDatabaseRequired(context, "ERP 외부 연동 로그 상세 조회");
  ...
  await recordOperationalPrivacyAccessEvent(context.env, {
    ...
    accessType: "read",
    purpose: "ERP 외부 연동 로그 상세 조회 감사 기록",
    legalBasis: "부서업무포털 권한 기반 경리 연동 로그 접근",
...
app.patch(ERP_INTEGRATION_EVENT_DETAIL_ROUTE, async (context) => {
  ...
  if (!before) return jsonDatabaseRequired(context, "ERP 외부 연동 로그 수정");
  ...
  if (!updated?.event) return jsonDatabaseRequired(context, "ERP 외부 연동 로그 수정");
...
app.patch(ERP_INTEGRATION_EVENT_STATUS_ROUTE, async (context) => {
  ...
  const before = await findOperationalErpIntegrationEvent(context.env, authResult.auth.user.companyId, eventId);
  if (!before) return jsonDatabaseRequired(context, "ERP 외부 연동 로그 상태 변경");
  if (!before.event) return jsonError(context, "FORBIDDEN", "허용되지 않은 연동 로그입니다.", 403, { eventId, route: context.req.path });
  ...
  if (!updated?.event) return jsonDatabaseRequired(context, "ERP 외부 연동 로그 상태 변경");
  ...
  const auditRecorded = await recordOperationalPrivacyAccessEvent(context.env, {
    ...
    resourceId: updated.event.id,
    accessType: "update",
    purpose: "ERP 외부 연동 로그 상태 변경 감사 기록",
    legalBasis: "부서업무포털 권한 기반 경리 연동 로그 처리",
    metadata: { source: "erp-integration-events-api", reason: parsed.data.reason, beforeStatus: before.event.status, afterStatus: updated.event.status },
  });
