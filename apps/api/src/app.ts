...
import {
  createRepairServiceFromBindings,
  type RepairBindings,
} from "./repairs/factory";
import { RepairServiceError, type RepairService } from "./repairs/service";
import {
  createOperationalIssueServiceFromBindings,
  type OperationalIssueBindings,
} from "./issues/factory";
import {
...
import { createKnowledgeServiceFromBindings } from "./knowledge/factory";
import {
...
} from "./daily-sales/factory";
import {
  DailySalesServiceError,
  type DailySalesService,
} from "./daily-sales/service";
import {
  createCalendarServiceFromBindings,
  type CalendarBindings,
} from "./calendars/factory";
import {
...
export function createApp(options: CreateAppOptions = {}) {
  ...
  function getOperationalIssueService(bindings: Bindings | undefined) {
    return (
      options.operationalIssueService ??
      createOperationalIssueServiceFromBindings(bindings)
    );
  }
  ...
  function getNotificationService(bindings: Bindings | undefined) {
    return (
      ...
      createNotificationServiceFromBindings(bindings)
    );
  }

  function getKnowledgeService(bindings: Bindings | undefined) {
    return (
      options.knowledgeService ?? createKnowledgeServiceFromBindings(bindings)
    );
  }
  ...
  function getCalendarService(bindings: Bindings | undefined) {
    return (
      options.calendarService ?? createCalendarServiceFromBindings(bindings)
    );
  }
  ...
  async function withKnowledgeService<T>(
    bindings: Bindings | undefined,
    operation: (service: KnowledgeService) => Promise<T>,
  ): Promise<T> {
    const service = getKnowledgeService(bindings);
    try {
      return await operation(service);
    } finally {
      if (!options.knowledgeService) await service.close?.();
    }
  }

  async function withDailySalesService<T>(
    bindings: Bindings | undefined,
    operation: (service: DailySalesService) => Promise<T>,
  ): Promise<T> {
    const service = getDailySalesService(bindings);
    try {
      return await operation(service);
    } finally {
      if (!options.dailySalesService) await service.close?.();
    }
  }
  ...
  function calendarRangeFailure(context: Context<{ Bindings: Bindings }>) {
    ...
    async (context) => {
      ...
      try {
        ...
        const routines = await withInspectionService(context.env, (service) =>
          service.listRoutinesV2(
            roomMutationPrincipal(context, principal),
            hotelId.data,
          ),
        );
        return context.json(
          inspectionRoutineV2ListResponseSchema.parse({
            ok: true,
            data: { routines },
            error: null,
          }),
  ...
  hotelApp.get(
    "/api/hotels/:hotelId/inspection-routines/v2/:routineId",
    async (context) => {
      context.header("Cache-Control", "no-store");
      try {
        const principal = await requestPrincipal(context);
        ...
        return context.json(
          inspectionRoutineV2ResponseSchema.parse({
            ok: true,
            data: { routine },
            error: null,
          }),
  ...
  const saveInspectionRoutineV2Route = async (
    context: Context,
    ...
    try {
      ...
      const parsed = createInspectionRoutineV2RequestSchema.safeParse(
        await context.req.json().catch(() => undefined),
      );
      if (!parsed.success)
        return validationFailure(context, zodFieldErrors(parsed.error.issues));
      const routine = await withInspectionService(context.env, (service) =>
        service.saveRoutineV2(
          roomMutationPrincipal(context, principal),
          hotelId.data,
          routineId,
          parsed.data,
  ...
  hotelApp.post("/api/hotels/:hotelId/inspection-routines/v2", (context) =>
    saveInspectionRoutineV2Route(context, null),
  );
  hotelApp.put(
    "/api/hotels/:hotelId/inspection-routines/v2/:routineId",
    (context) => {
  ...
  hotelApp.get("/api/hotels/:hotelId/inspections/v2", async (context) => {
    ...
    try {
      ...
      return context.json(
        inspectionExecutionV2ListResponseSchema.parse({
          ok: true,
          data,
          error: null,
        }),
  ...
  hotelApp.post(
    "/api/hotels/:hotelId/inspections/v2/manual",
    async (context) => {
      context.header("Cache-Control", "no-store");
      try {
        const principal = await requestPrincipal(context);
        ...
        const parsed = createManualInspectionV2RequestSchema.safeParse(
          await context.req.json().catch(() => undefined),
        );
        if (!parsed.success)
          return validationFailure(
        ...
        return context.json(
          inspectionExecutionV2ResponseSchema.parse({
            ok: true,
            data: { inspection },
            error: null,
          }),
  ...
  hotelApp.get(
    "/api/hotels/:hotelId/inspections/v2/:inspectionId",
    async (context) => {
      context.header("Cache-Control", "no-store");
      try {
        const principal = await requestPrincipal(context);
  ...
  hotelApp.get(
    ...
    async (context) => {
      ...
      try {
        ...
        const hotelId = HOTEL_ID_SCHEMA.safeParse(context.req.param("hotelId"));
        const salesId = z.uuid().safeParse(context.req.param("salesId"));
        const fileVersionId = z
          .uuid()
          .safeParse(context.req.param("fileVersionId"));
        if (!hotelId.success || !salesId.success || !fileVersionId.success)
          return mutationFailure(context, "NOT_FOUND");
        service = getHotelFileService(context.env);
        const view = await service.dailySalesView(
          roomMutationPrincipal(context, principal),
          hotelId.data,
          salesId.data,
          fileVersionId.data,
  ...
  hotelApp.get(
    ...
    async (context) => {
      ...
      try {
        ...
        const hotelId = HOTEL_ID_SCHEMA.safeParse(context.req.param("hotelId")),
          inquiryId = z.uuid().safeParse(context.req.param("inquiryId")),
          fileVersionId = z
            .uuid()
            .safeParse(context.req.param("fileVersionId"));
        if (!hotelId.success || !inquiryId.success || !fileVersionId.success)
          return mutationFailure(context, "NOT_FOUND");
        service = getHotelFileService(context.env);
        const view = await service.inquiryView(
          roomMutationPrincipal(context, principal),
          hotelId.data,
          inquiryId.data,
          fileVersionId.data,
  ...
  hotelApp.get("/api/calendar/capabilities", async (context) => {
    ...
    try {
      ...
      return context.json(
        calendarCapabilitiesResponseSchema.parse({
          ok: true,
          data,
          error: null,
        }),
  ...
  hotelApp.get("/api/calendar", async (context) => {
    ...
    try {
      ...
      if (!query.success) {
        ...
        return (
          rangeFailure ??
          validationFailure(context, zodFieldErrors(query.error.issues))
        );
      }
      const data = await withCalendarService(context.env, (service) =>
      ...
      return context.json(
        calendarEventsResponseSchema.parse({ ok: true, data, error: null }),
      );
    } catch (error) {
      if (error instanceof AuthServiceError) return authFailure(context, error);
      return hotelFailure(context, error);
  ...
  hotelApp.get(
    ...
    async (context) => {
      ...
      try {
        ...
        if (!hotelId.success) return mutationFailure(context, "NOT_FOUND");
        const data = await withCalendarService(context.env, (service) =>
          service.getVisitOptions(
            roomMutationPrincipal(context, principal),
            hotelId.data,
          ),
        );
        return context.json(
          calendarVisitOptionsResponseSchema.parse({
            ok: true,
            data,
            error: null,
          }),
  ...
  hotelApp.get("/api/hotels/:hotelId/repairs", async (context) => {
    ...
    try {
      ...
      const hotelId = HOTEL_ID_SCHEMA.safeParse(context.req.param("hotelId"));
      const query = repairListQuerySchema.safeParse(context.req.query());
      if (!hotelId.success) return mutationFailure(context, "NOT_FOUND");
      if (!query.success)
        return validationFailure(context, zodFieldErrors(query.error.issues));
      ...
      return context.json(
        repairListResponseSchema.parse({ ok: true, data, error: null }),
      );
    } catch (error) {
      if (error instanceof AuthServiceError) return authFailure(context, error);
      return hotelFailure(context, error);
  ...
  hotelApp.post("/api/hotels/:hotelId/repairs", async (context) => {
    ...
    try {
      ...
      const hotelId = HOTEL_ID_SCHEMA.safeParse(context.req.param("hotelId"));
      const key = idempotencyKey(context);
      const body = createRepairCaseRequestSchema.safeParse(
        await context.req.json().catch(() => undefined),
      );
      if (!hotelId.success) return mutationFailure(context, "NOT_FOUND");
      if (!key)
      ...
      return context.json(
        repairCaseResponseSchema.parse({
          ok: true,
          data: { repair },
          error: null,
        }),
  ...
  hotelApp.get("/api/hotels/:hotelId/repairs/:repairId", async (context) => {
    ...
    try {
      ...
      const hotelId = HOTEL_ID_SCHEMA.safeParse(context.req.param("hotelId"));
      const repairId = z.uuid().safeParse(context.req.param("repairId"));
      if (!hotelId.success || !repairId.success)
        return mutationFailure(context, "NOT_FOUND");
      const repair = await withRepairService(context.env, (service) =>
        service.getRepair(
          roomMutationPrincipal(context, principal),
  ...
  hotelApp.get(
    ...
    async (context) => {
      ...
      try {
        ...
        return context.json(
          repairFollowUpListResponseSchema.parse({
            ok: true,
            data,
            error: null,
          }),
  ...
  hotelApp.post("/api/hotels/:hotelId/repair-visits", async (context) => {
    ...
    try {
      ...
      const hotelId = HOTEL_ID_SCHEMA.safeParse(context.req.param("hotelId"));
      const key = idempotencyKey(context);
      const body = createRepairVisitRequestSchema.safeParse(
        await context.req.json().catch(() => undefined),
      );
      if (!hotelId.success) return mutationFailure(context, "NOT_FOUND");
      if (!key)
      ...
      if (!body.success)
        return validationFailure(context, zodFieldErrors(body.error.issues));
      const visit = await withRepairService(context.env, (service) =>
        service.createVisit(
          roomMutationPrincipal(context, principal),
          hotelId.data,
          body.data,
      ...
      return context.json({ ok: true, data: { visit }, error: null }, 201);
    } catch (error) {
      if (error instanceof AuthServiceError) return authFailure(context, error);
      return hotelFailure(context, error);
    }
  ...
  const repairVisitMutation = async (
    context: Context<{ Bindings: Bindings }>,
    action: "UPDATE" | "CANCEL" | "RESTORE" | "DELETE" | "COMPLETE",
  ) => {
    context.header("Cache-Control", "no-store");
    try {
      const principal = await requestPrincipal(context);
      ...
      const hotelId = HOTEL_ID_SCHEMA.safeParse(context.req.param("hotelId"));
      const visitId = z.uuid().safeParse(context.req.param("visitId"));
      const key = idempotencyKey(context);
      const raw = await context.req.json().catch(() => undefined);
      const body =
        action === "UPDATE"
          ? updateRepairVisitRequestSchema.safeParse(raw)
          : action === "COMPLETE"
            ? completeRepairVisitRequestSchema.safeParse(raw)
            : repairVersionReasonRequestSchema.safeParse(raw);
      if (!hotelId.success || !visitId.success)
        return mutationFailure(context, "NOT_FOUND");
      if (!key)
        return validationFailure(context, [
          {
      ...
      return context.json({ ok: true, data: { visit }, error: null });
    } catch (error) {
      if (error instanceof AuthServiceError) return authFailure(context, error);
      return hotelFailure(context, error);
    }
  };
  hotelApp.patch("/api/hotels/:hotelId/repair-visits/:visitId", (context) =>
    repairVisitMutation(context, "UPDATE"),
  );
  hotelApp.post(
    "/api/hotels/:hotelId/repair-visits/:visitId/cancel",
    (context) => repairVisitMutation(context, "CANCEL"),
  );
  hotelApp.post(
    "/api/hotels/:hotelId/repair-visits/:visitId/restore",
    (context) => repairVisitMutation(context, "RESTORE"),
  );
  hotelApp.post(
    "/api/hotels/:hotelId/repair-visits/:visitId/delete",
    (context) => repairVisitMutation(context, "DELETE"),
  );
  hotelApp.post(
    "/api/hotels/:hotelId/repair-visits/:visitId/complete",
    (context) => repairVisitMutation(context, "COMPLETE"),
  );
  ...
  hotelApp.post(
    ...
    async (context) => {
      ...
      try {
        ...
        const repairId = z.uuid().safeParse(context.req.param("repairId"));
        const key = idempotencyKey(context);
        const body = submitRepairReviewRequestSchema.safeParse(
          await context.req.json().catch(() => undefined),
        );
        if (!hotelId.success || !repairId.success)
          return mutationFailure(context, "NOT_FOUND");
  ...
  hotelApp.post(
    ...
    async (context) => {
      ...
      try {
        ...
        const repairId = z.uuid().safeParse(context.req.param("repairId"));
        const key = idempotencyKey(context);
        const body = repairProcessTransitionRequestSchema.safeParse(
          await context.req.json().catch(() => undefined),
        );
        if (!hotelId.success || !repairId.success)
          return mutationFailure(context, "NOT_FOUND");
  ...
  hotelApp.post(
    ...
    async (context) => {
      ...
      try {
        ...
        const repairId = z.uuid().safeParse(context.req.param("repairId"));
        const key = idempotencyKey(context);
        const body = completeRepairCaseRequestSchema.safeParse(
          await context.req.json().catch(() => undefined),
        );
        if (!hotelId.success || !repairId.success)
          return mutationFailure(context, "NOT_FOUND");
  ...
  function dailySalesResponse(sales: unknown) {
    ...
    if (internal.success) return internal.data;
    return dailySalesOwnerResponseSchema.parse({
      ok: true,
      data: { sales },
      error: null,
    });
  ...
  hotelApp.get("/api/daily-sales/capabilities", async (context) => {
    context.header("Cache-Control", "private, no-store");
    try {
      const principal = await requestPrincipal(context);
      if (!principal)
      ...
      const data = await withDailySalesService(context.env, (service) =>
        service.capabilities(roomMutationPrincipal(context, principal)),
      );
      return context.json(
        dailySalesCapabilitiesResponseSchema.parse({
          ok: true,
          data,
          error: null,
        }),
  ...
  hotelApp.get(
    ...
    async (context) => {
      ...
      try {
        ...
        return context.json(
          dailySalesReferenceResponseSchema.parse({
            ok: true,
            data,
            error: null,
          }),
  ...
  hotelApp.get("/api/hotels/:hotelId/daily-sales", async (context) => {
    context.header("Cache-Control", "private, no-store");
    try {
      const principal = await requestPrincipal(context);
      if (!principal)
      ...
      const hotelId = HOTEL_ID_SCHEMA.safeParse(context.req.param("hotelId"));
      const query = dailySalesListQuerySchema.safeParse(context.req.query());
      if (!hotelId.success) return mutationFailure(context, "NOT_FOUND");
      if (!query.success)
        return validationFailure(context, zodFieldErrors(query.error.issues));
      const data = await withDailySalesService(context.env, (service) =>
      ...
      return context.json(
        dailySalesListResponseSchema.parse({ ok: true, data, error: null }),
      );
    } catch (error) {
      if (error instanceof AuthServiceError) return authFailure(context, error);
      return hotelFailure(context, error);
  ...
  hotelApp.post("/api/hotels/:hotelId/daily-sales", async (context) => {
    context.header("Cache-Control", "no-store");
    try {
      const principal = await requestPrincipal(context);
      if (!principal)
      ...
      const hotelId = HOTEL_ID_SCHEMA.safeParse(context.req.param("hotelId"));
      const key = idempotencyKey(context);
      const body = createDailySalesDraftRequestSchema.safeParse(
        await context.req.json().catch(() => undefined),
      );
      if (!hotelId.success) return mutationFailure(context, "NOT_FOUND");
      if (!key)
      ...
      if (!body.success)
        return validationFailure(context, zodFieldErrors(body.error.issues));
      const sales = await withDailySalesService(context.env, (service) =>
        service.create(
          roomMutationPrincipal(context, principal),
          hotelId.data,
          body.data,
      ...
      return context.json(dailySalesResponse(sales), 201);
    } catch (error) {
      if (error instanceof AuthServiceError) return authFailure(context, error);
      return hotelFailure(context, error);
    }
  ...
  hotelApp.get("/api/hotels/:hotelId/daily-sales/:salesId", async (context) => {
    context.header("Cache-Control", "private, no-store");
    try {
      const principal = await requestPrincipal(context);
      if (!principal)
      ...
      const hotelId = HOTEL_ID_SCHEMA.safeParse(context.req.param("hotelId"));
      const salesId = z.uuid().safeParse(context.req.param("salesId"));
      if (!hotelId.success || !salesId.success)
        return mutationFailure(context, "NOT_FOUND");
      const sales = await withDailySalesService(context.env, (service) =>
        service.get(
          roomMutationPrincipal(context, principal),
      ...
      return context.json(dailySalesResponse(sales));
    } catch (error) {
      if (error instanceof AuthServiceError) return authFailure(context, error);
      return hotelFailure(context, error);
    }
  ...
  hotelApp.patch(
    ...
    async (context) => {
      ...
      try {
        ...
        const salesId = z.uuid().safeParse(context.req.param("salesId"));
        const key = idempotencyKey(context);
        const body = updateDailySalesDraftRequestSchema.safeParse(
          await context.req.json().catch(() => undefined),
        );
        if (!hotelId.success || !salesId.success)
          return mutationFailure(context, "NOT_FOUND");
  ...
  async function dailySalesAction(
    context: Context<{ Bindings: Bindings }>,
    ...
    try {
      ...
      const body =
        action === "confirm"
          ? confirmDailySalesRequestSchema.safeParse(raw)
          : correctDailySalesRequestSchema.safeParse(raw);
      if (!hotelId.success || !salesId.success)
        return mutationFailure(context, "NOT_FOUND");
      if (!key)
        return validationFailure(context, [
  ...
  hotelApp.post(
    ...
    (context) => dailySalesAction(context, "confirm"),
  );
  hotelApp.post(
  ...
  hotelApp.get("/api/notifications", async (context) => {
    ...
    try {
      ...
      const query = hotelNotificationListQuerySchema.safeParse(
        context.req.query(),
      );
      if (!query.success)
        return validationFailure(context, zodFieldErrors(query.error.issues));
      ...
      return context.json(
        hotelNotificationListResponseSchema.parse({
          ok: true,
          data,
          error: null,
        }),
  ...
  hotelApp.post("/api/notifications/:notificationId/read", async (context) => {
    ...
    try {
      ...
      const notificationId = z
        .uuid()
        .safeParse(context.req.param("notificationId"));
      const key = idempotencyKey(context);
      ...
      const notification = await withNotificationService(
        context.env,
        (service) =>
          service.markRead(
  ...
  function inquiryResponse(inquiry: unknown) {
    ...
    if (internal.success) return internal.data;
    return hotelInquiryOwnerResponseSchema.parse({
      ok: true,
      data: { inquiry },
      error: null,
    });
  ...
  hotelApp.get("/api/inquiries/capabilities", async (context) => {
    ...
    try {
      ...
      const data = await withInquiryService(context.env, (s) =>
        s.capabilities(roomMutationPrincipal(context, p)),
      );
      return context.json(
        hotelInquiryCapabilitiesResponseSchema.parse({
          ok: true,
          data,
          error: null,
        }),
  ...
  hotelApp.get("/api/hotels/:hotelId/inquiry-contact", async (context) => {
    ...
    try {
      ...
      const h = HOTEL_ID_SCHEMA.safeParse(context.req.param("hotelId"));
      if (!h.success) return mutationFailure(context, "NOT_FOUND");
      const data = await withInquiryService(context.env, (s) =>
        s.contact(roomMutationPrincipal(context, p), h.data),
      );
      return context.json(
        hotelInquiryContactResponseSchema.parse({
          ok: true,
          data,
          error: null,
        }),
  ...
  hotelApp.get("/api/hotels/:hotelId/inquiry-settings", async (context) => {
    ...
    try {
      const p = await requestPrincipal(context),
        h = HOTEL_ID_SCHEMA.safeParse(context.req.param("hotelId"));
      if (!p)
        return context.json(
          errorResponse(
            "AUTHENTICATION_REQUIRED",
      ...
      if (!h.success) return mutationFailure(context, "NOT_FOUND");
      const data = await withInquiryService(context.env, (s) =>
        s.settings(roomMutationPrincipal(context, p), h.data),
      );
      return context.json(
        hotelInquirySettingsResponseSchema.parse({
          ok: true,
          data,
          error: null,
        }),
  ...
  hotelApp.put(
    "/api/hotels/:hotelId/inquiry-settings/contact",
    async (context) => {
      context.header("Cache-Control", "no-store");
      try {
        const p = await requestPrincipal(context),
          ...
          body = updateHotelInquiryContactRequestSchema.safeParse(
            await context.req.json().catch(() => undefined),
          );
        if (!p)
          return context.json(
  ...
  hotelApp.put(
    "/api/hotels/:hotelId/inquiry-settings/routes/:categoryCode",
    async (context) => {
      context.header("Cache-Control", "no-store");
      try {
        const p = await requestPrincipal(context),
          h = HOTEL_ID_SCHEMA.safeParse(context.req.param("hotelId")),
          category = hotelInquiryCategoryCodeSchema.safeParse(
            context.req.param("categoryCode"),
          ),
          key = idempotencyKey(context),
          body = updateHotelInquiryRouteRequestSchema.safeParse(
            await context.req.json().catch(() => undefined),
          );
        if (!p)
          return context.json(
  ...
  hotelApp.get("/api/hotels/:hotelId/inquiries", async (context) => {
    ...
    try {
      ...
      const h = HOTEL_ID_SCHEMA.safeParse(context.req.param("hotelId"));
      const q = hotelInquiryListQuerySchema.safeParse(context.req.query());
      if (!h.success) return mutationFailure(context, "NOT_FOUND");
      if (!q.success)
        return validationFailure(context, zodFieldErrors(q.error.issues));
      const data = await withInquiryService(context.env, (s) =>
        s.list(roomMutationPrincipal(context, p), h.data, q.data),
      );
      return context.json(
        hotelInquiryListResponseSchema.parse({ ok: true, data, error: null }),
      );
    } catch (error) {
      if (error instanceof AuthServiceError) return authFailure(context, error);
      return hotelFailure(context, error);
  ...
  hotelApp.post("/api/hotels/:hotelId/inquiries", async (context) => {
    ...
    try {
      ...
      const h = HOTEL_ID_SCHEMA.safeParse(context.req.param("hotelId"));
      const key = idempotencyKey(context);
      const body = createHotelInquiryRequestSchema.safeParse(
        await context.req.json().catch(() => undefined),
      );
      if (!h.success) return mutationFailure(context, "NOT_FOUND");
      if (!key)
      ...
      if (!body.success)
        return validationFailure(context, zodFieldErrors(body.error.issues));
      const inquiry = await withInquiryService(context.env, (s) =>
        s.create(roomMutationPrincipal(context, p), h.data, body.data, key),
      );
      return context.json(inquiryResponse(inquiry), 201);
    } catch (error) {
      if (error instanceof AuthServiceError) return authFailure(context, error);
      return hotelFailure(context, error);
    }
  ...
  hotelApp.get("/api/hotels/:hotelId/inquiries/:inquiryId", async (context) => {
    ...
    try {
      ...
      const h = HOTEL_ID_SCHEMA.safeParse(context.req.param("hotelId")),
        i = z.uuid().safeParse(context.req.param("inquiryId"));
      if (!h.success || !i.success)
        return mutationFailure(context, "NOT_FOUND");
      const inquiry = await withInquiryService(context.env, (s) =>
        s.get(roomMutationPrincipal(context, p), h.data, i.data),
      );
      return context.json(inquiryResponse(inquiry));
    } catch (error) {
      if (error instanceof AuthServiceError) return authFailure(context, error);
      return hotelFailure(context, error);
    }
  });
  async function inquiryMutation(
    context: Context<{ Bindings: Bindings }>,
    kind: "MESSAGE" | "ASSIGN" | "TRANSITION",
  ) {
    context.header("Cache-Control", "no-store");
    try {
      const p = await requestPrincipal(context);
      ...
      const h = HOTEL_ID_SCHEMA.safeParse(context.req.param("hotelId")),
        i = z.uuid().safeParse(context.req.param("inquiryId")),
        key = idempotencyKey(context),
        raw = await context.req.json().catch(() => undefined);
      const body =
        kind === "ASSIGN"
          ? hotelInquiryAssignRequestSchema.safeParse(raw)
          : kind === "TRANSITION"
            ? hotelInquiryTransitionRequestSchema.safeParse(raw)
            : hotelInquiryMessageRequestSchema.safeParse(raw);
      if (!h.success || !i.success)
        return mutationFailure(context, "NOT_FOUND");
      if (!key)
        return validationFailure(context, [
  ...
  hotelApp.post("/api/hotels/:hotelId/inquiries/:inquiryId/messages", (c) =>
    inquiryMutation(c, "MESSAGE"),
  );
  hotelApp.post("/api/hotels/:hotelId/inquiries/:inquiryId/assign", (c) =>
    inquiryMutation(c, "ASSIGN"),
  );
  hotelApp.post("/api/hotels/:hotelId/inquiries/:inquiryId/transitions", (c) =>
    inquiryMutation(c, "TRANSITION"),
  );
  ...
  hotelApp.get(knowledgeRoutes.capabilities, async (context) => {
    context.header("Cache-Control", "private, no-store");
    try {
      const actor = await requestPrincipal(context);
      if (!actor)
        return context.json(
          errorResponse(
            "AUTHENTICATION_REQUIRED",
      ...
      const data = await withKnowledgeService(context.env, (service) =>
        service.capabilities(roomMutationPrincipal(context, actor)),
      );
      return context.json(
        knowledgeCapabilitiesResponseSchema.parse({
          ok: true,
          data,
          error: null,
        }),
  ...
  hotelApp.get(knowledgeRoutes.reviewerCandidates, async (context) => {
    context.header("Cache-Control", "private, no-store");
    try {
      const actor = await requestPrincipal(context);
      if (!actor)
      ...
      const query = knowledgeReviewerCandidatesQuerySchema.safeParse(
        context.req.query(),
      );
      if (!query.success)
        return validationFailure(context, zodFieldErrors(query.error.issues));
      const data = await withKnowledgeService(context.env, (service) =>
        service.reviewerCandidates(
          roomMutationPrincipal(context, actor),
          query.data,
        ),
      );
      return context.json(
        knowledgeReviewerCandidatesResponseSchema.parse({
          ok: true,
          data,
          error: null,
        }),
  ...
  hotelApp.get(knowledgeRoutes.list, async (context) => {
    context.header("Cache-Control", "private, no-store");
    try {
      const actor = await requestPrincipal(context);
      if (!actor)
      ...
      const query = knowledgeListQuerySchema.safeParse(context.req.query());
      if (!query.success)
        return validationFailure(context, zodFieldErrors(query.error.issues));
      const data = await withKnowledgeService(context.env, (service) =>
        service.list(roomMutationPrincipal(context, actor), query.data),
      );
      return context.json(
        knowledgeListResponseSchema.parse({ ok: true, data, error: null }),
      );
    } catch (error) {
      return hotelFailure(context, error);
    }
  ...
  hotelApp.post(knowledgeRoutes.create, async (context) => {
    context.header("Cache-Control", "private, no-store");
    try {
      const actor = await requestPrincipal(context);
      if (!actor)
      ...
      const entry = await withKnowledgeService(context.env, (service) =>
        service.create(roomMutationPrincipal(context, actor), body, key),
      );
      return context.json(
  ...
  hotelApp.post("/api/knowledge/:knowledgeId/files/upload-init", async (context) => {
    context.header("Cache-Control", "private, no-store");
    try {
      const actor = await requestPrincipal(context);
      if (!actor)
      ...
      const knowledgeId = z.uuid().safeParse(context.req.param("knowledgeId"));
      if (!knowledgeId.success) return mutationFailure(context, "NOT_FOUND");
      const key = idempotencyKey(context);
      if (!key)
        return validationFailure(context, [{ field: "idempotencyKey", message: "Idempotency-Key 헤더가 필요합니다." }]);
      const parsed = hotelFileUploadInitRequestSchema.safeParse(await context.req.json());
      if (
        !parsed.success ||
        parsed.data.parent.type !== "KNOWLEDGE_ATTACHMENT" ||
        parsed.data.parent.knowledgeId !== knowledgeId.data
      )
        return mutationFailure(context, "NOT_FOUND");
      const data = await withHotelFileService(context.env, (service) =>
      ...
      return context.json(
        hotelFileUploadInitResponseSchema.parse({ ok: true, data, error: null }),
        201,
      );
    } catch (error) {
      return hotelFailure(context, error);
  ...
  hotelApp.get(
    "/api/knowledge/:knowledgeId/files/uploads/:uploadId/status",
    async (context) => {
      context.header("Cache-Control", "private, no-store");
      try {
        const actor = await requestPrincipal(context);
        ...
        const knowledgeId = z.uuid().safeParse(context.req.param("knowledgeId"));
        const uploadId = z.uuid().safeParse(context.req.param("uploadId"));
        if (!knowledgeId.success || !uploadId.success)
          return mutationFailure(context, "NOT_FOUND");
        const upload = await withHotelFileService(context.env, (service) =>
  ...
  hotelApp.put("/api/knowledge/:knowledgeId/attachments", async (context) => {
    context.header("Cache-Control", "private, no-store");
    try {
      const actor = await requestPrincipal(context);
      if (!actor)
      ...
      return context.json(
        knowledgeEntryResponseSchema.parse({ ok: true, data: { entry }, error: null }),
      );
    } catch (error) {
      return hotelFailure(context, error);
    }
  ...
  hotelApp.get(
    ...
    async (context) => {
      ...
      try {
        ...
        const knowledgeId = z.uuid().safeParse(context.req.param("knowledgeId"));
        const fileVersionId = z.uuid().safeParse(context.req.param("fileVersionId"));
        if (!knowledgeId.success || !fileVersionId.success)
          return mutationFailure(context, "NOT_FOUND");
        service = getHotelFileService(context.env);
        const view = await service.knowledgeView(
          roomMutationPrincipal(context, actor),
          knowledgeId.data,
          fileVersionId.data,
        );
  ...
  hotelApp.get("/api/knowledge/:knowledgeId", async (context) => {
    context.header("Cache-Control", "private, no-store");
    try {
      const actor = await requestPrincipal(context);
      if (!actor)
      ...
      if (!knowledgeId.success) return mutationFailure(context, "NOT_FOUND");
      const entry = await withKnowledgeService(context.env, (service) =>
        service.get(roomMutationPrincipal(context, actor), knowledgeId.data),
      );
      return context.json(
        knowledgeEntryResponseSchema.parse({
          ok: true,
  ...
  hotelApp.patch("/api/knowledge/:knowledgeId", async (context) => {
    context.header("Cache-Control", "private, no-store");
    try {
      const actor = await requestPrincipal(context);
      if (!actor)
  ...
  hotelApp.post("/api/knowledge/:knowledgeId/transitions", async (context) => {
    context.header("Cache-Control", "private, no-store");
    try {
      const actor = await requestPrincipal(context);
      if (!actor)
  ...
  hotelApp.post("/api/knowledge/:knowledgeId/feedback", async (context) => {
    context.header("Cache-Control", "private, no-store");
    try {
      const actor = await requestPrincipal(context);
      if (!actor)
      ...
      return context.json(
        knowledgeFeedbackResponseSchema.parse({ ok: true, data, error: null }),
      );
    } catch (error) {
      return hotelFailure(context, error);
    }
  ...
  hotelApp.get("/api/knowledge/:knowledgeId/links", async (context) => {
    context.header("Cache-Control", "private, no-store");
    try {
      const actor = await requestPrincipal(context);
      if (!actor)
