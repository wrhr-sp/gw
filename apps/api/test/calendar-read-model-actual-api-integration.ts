...
function repeatedFixture(source: string, name: string) {
  ...
  if (!match?.[1] || !match[2])
    throw new Error(`missing token fixture ${name}`);
...
async function expectStatus(response: Response, status: number, label: string) {
  ...
  throw new Error(
    `${label}: expected ${status}, received ${response.status} ${JSON.stringify(payload)}`,
  );
}
...
async function main() {
  ...
  if (!databaseUrl || !adminUrl || !fixturePath)
    throw new Error("Calendar actual API environment is incomplete");
  ...
  const postgres = requireFromDb(["post", "gres"].join("")) as (
    url: string,
  ...
  const app = createApp({
    authService,
    calendarService: createCalendarService(repository),
  });
  try {
    ...
    const capabilities = calendarCapabilitiesResponseSchema.parse(
      await capabilitiesResponse.json(),
    ).data;
    if (
      !capabilities.canViewAllHotels ||
      !capabilities.hotels.some(
        (hotel) => hotel.id === hotelId && hotel.canCreateVisit,
      )
    )
      throw new Error("Calendar capabilities did not preserve hotel scope");

    const range = "from=2026-08-01&to=2026-09-12&pageSize=200";
    const hotelResponse = await request(
      app,
      `/api/hotels/${hotelId}/calendar?${range}`,
      token,
    );
    await expectStatus(hotelResponse, 200, "single hotel Calendar");
    const hotelCalendar = calendarEventsResponseSchema.parse(
      await hotelResponse.json(),
    ).data;
    const repairVisit = hotelCalendar.events.find(
      (event) => event.type === "REPAIR_VISIT",
    );
    if (!repairVisit) throw new Error("canonical repair visit is missing");
    ...
    const allCalendar = calendarEventsResponseSchema.parse(
    ...
    const options = calendarVisitOptionsResponseSchema.parse(
      await optionsResponse.json(),
    ).data;
    if (
      !options.repairs.length ||
      !options.internalPerformers.some(
        (person) => person.userId === principal.userId,
      )
    )
    ...
    const oversized = await request(
      app,
      `/api/hotels/${hotelId}/calendar?from=2026-08-01&to=2026-09-13&pageSize=200`,
      token,
    );
    await expectStatus(oversized, 400, "42-day range boundary");
    const oversizedError = hotelErrorResponseSchema.parse(
      await oversized.json(),
    );
    if (oversizedError.error.code !== "CALENDAR_RANGE_TOO_LARGE")
      throw new Error(
        "oversized range did not return the stable Calendar error",
      );

    const invalidCursor = await request(
      app,
      `/api/hotels/${hotelId}/calendar?${range}&cursor=not_hex`,
      token,
    );
    ...
    const forbidden = await request(
      app,
      `/api/hotels/50000000-0000-4000-8000-000000000099/calendar?${range}`,
      token,
    );
    ...
    await fixtureSql.begin(async (transaction) => {
      ...
      await transaction`
        insert into public.housekeeping_hotel_links(
          ...
          pg_catalog.gen_random_uuid(), ${companyId}::uuid,
    ...
    try {
      const terminatedCalendar = await request(
        app,
        `/api/hotels/${hotelId}/calendar?${range}`,
        token,
      );
      await expectStatus(
        terminatedCalendar,
        403,
        "terminated housekeeping Calendar denial",
      );
      const terminatedOptions = await request(
        app,
        `/api/hotels/${hotelId}/calendar/visit-options`,
        token,
      );
      await expectStatus(
        terminatedOptions,
        403,
        "terminated housekeeping visit-options denial",
      );
    } finally {
    ...
    if (denseVisits.length !== 5001)
      throw new Error(
        `Calendar density fixture inserted ${denseVisits.length} visits`,
      );
    const dense = await request(
      app,
      `/api/hotels/${hotelId}/calendar?${range}`,
      token,
    );
    ...
    if (denseError.error.code !== "CALENDAR_RESULT_TOO_DENSE")
      throw new Error("dense range did not return the stable Calendar error");
    console.log("HOTEL_CALENDAR_ACTUAL_API_INTEGRATION_OK");
  } finally {
    await fixtureSql.begin(async (transaction) => {
      await transaction`
          delete from public.hotel_repair_visit_performers performer
           using public.hotel_repair_visits visit
           where performer.company_id = visit.company_id
             and performer.branch_id = visit.branch_id
             and performer.repair_visit_id = visit.id
             and visit.company_id = ${companyId}::uuid
             and visit.branch_id = ${hotelId}::uuid
             and visit.title = 'Calendar density guard probe'
        `;
      await transaction`
        delete from public.hotel_repair_visits
         where company_id = ${companyId}::uuid
           and branch_id = ${hotelId}::uuid
           and title = 'Calendar density guard probe'
      `;
    });
