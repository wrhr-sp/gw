...
describe("ZITADEL account provisioning provider", () => {
  it("creates a human identity with a caller-selected deterministic ID", async () => {
    ...
    expect(init?.headers).toMatchObject({ authorization: "Bearer provider-token-value" });
  ...
  it("reads back only the deterministic user ID after an ambiguous create result", async () => {
    ...
    const lifecycle = provider(fetcher as typeof fetch) as ReturnType<typeof provider> & {
      humanUserExists(id: string): Promise<boolean>;
    };
    await expect(lifecycle.humanUserExists(userId)).resolves.toBe(true);
    expect(fetcher).toHaveBeenCalledWith(`${issuer}/v2/users/${userId}`, expect.objectContaining({
      method: "GET",
      redirect: "manual",
    }));
  });
  ...
  it("fails closed when deterministic read-back belongs to another organization", async () => {
    ...
    await expect(provider(fetcher as typeof fetch).humanUserExists(userId))
  ...
  it("deactivates a provider identity for compensation or account suspension", async () => {
    ...
    await provider(fetcher as typeof fetch).deactivateHumanUser(userId);
    expect(fetcher).toHaveBeenCalledWith(`${issuer}/v2/users/${userId}/deactivate`, expect.objectContaining({
      method: "POST",
      redirect: "manual",
    }));
  });

  it("converges when the provider identity is already absent", async () => {
    const fetcher = vi.fn<typeof fetch>(async () => new Response(JSON.stringify({}), {
      status: 404,
      headers: { "content-type": "application/json" },
    }));
    await expect(provider(fetcher as typeof fetch).deactivateHumanUser(userId)).resolves.toBeUndefined();
  });
  ...
  it("verifies a credential with the separate login token and closes the temporary provider session", async () => {
    const fetcher = vi.fn<typeof fetch>(async (url, init) => {
      if (String(url) === `${issuer}/v2/sessions` && init?.method === "POST") {
        return new Response(JSON.stringify({ sessionId: "verification-session", sessionToken: "verification-session-token" }), {
          status: 200,
          headers: { "content-type": "application/json" },
        });
      }
      if (String(url) === `${issuer}/v2/sessions/verification-session` && init?.method === "GET") {
    ...
    expect(sessionCreate[1]?.headers).toMatchObject({ authorization: "Bearer login-verification-token-value" });
    expect(JSON.parse(String(sessionCreate[1]?.body))).toEqual({
      checks: { user: { loginName: "housekeeper-01" }, password: { password: "Strong-Preview-123!" } },
      lifetime: "300s",
    });
    expect(fetcher).toHaveBeenCalledWith(`${issuer}/v2/sessions/verification-session`, expect.objectContaining({ method: "DELETE" }));
  });
  ...
  it("sets the authenticated user's first private password without persisting it locally", async () => {
    ...
    await provider(fetcher as typeof fetch).setPassword(userId, "Another-Strong-456!");
    const [url, init] = fetcher.mock.calls[0]!;
