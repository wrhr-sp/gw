...
describe("hosted Preview Console credential smoke", () => {
  ...
  it("rejects callback, login, signed-out, error and error-query landing routes", () => {
    const issuerOrigin = "https://identity.example.test";
    expect(
      isValidConsoleLanding(`${issuerOrigin}/ui/console/`, issuerOrigin),
    ).toBe(true);
    expect(
      isValidConsoleLanding(
        `${issuerOrigin}/ui/console/projects`,
        issuerOrigin,
      ),
    ).toBe(true);
    for (const url of [
  ...
  it("classifies OIDC token identity failures without exposing claim values", () => {
    ...
    expect(
      consoleTokenIdentityFailureStage({ claims: validClaims, ...expected }),
    ).toBeNull();
    expect(
      consoleTokenIdentityFailureStage({ claims: null, ...expected }),
    ).toBe("TOKEN_DECODE");
    expect(
      consoleTokenIdentityFailureStage({
        claims: { ...validClaims, iss: "https://other.example.test" },
        ...expected,
      }),
    ).toBe("TOKEN_ISSUER");
    expect(
    ...
    expect(
      consoleTokenIdentityFailureStage({
        claims: { ...validClaims, sub: "other-subject" },
        ...expected,
      }),
    ).toBe("TOKEN_SUBJECT");
    expect(
      consoleTokenIdentityFailureStage({
        claims: { ...validClaims, exp: 1_000 },
        ...expected,
      }),
    ).toBe("TOKEN_EXPIRY");
  });
  ...
  it("classifies credential completion failures with a fixed secret-safe stage", () => {
    ...
    expect(
      consoleCredentialFailureStage({
        ...successful,
        authenticatedUserStatus: "rejected",
      }),
    ...
    expect(
      consoleCredentialFailureStage({ ...successful, identityVerified: false }),
    ).toBe("TOKEN_IDENTITY");
    expect(
      consoleCredentialFailureStage({ ...successful, cookieCleared: false }),
    ).toBe("COOKIE_CLEANUP");
    expect(consoleCredentialFailureMarker("CALLBACK_RESPONSE")).toBe(
      "ZITADEL_CONSOLE_PREVIEW_CREDENTIAL_FAILED_CALLBACK_RESPONSE",
    );
    expect(consoleCredentialFailureMarker("secret-sentinel")).toBe(
      "ZITADEL_CONSOLE_PREVIEW_CREDENTIAL_FAILED_UNCLASSIFIED",
    );
    const fulfilled = { status: "fulfilled", value: undefined } as const;
    const rejected = {
      status: "rejected",
      reason: new Error("listener failed"),
    } as const;
    expect(
  ...
  it("keeps cookie cleanup failure fixed and prevents browser close from overwriting it", () => {
    ...
    expect(`${bothFailed.stdout}${bothFailed.stderr}`).not.toContain(
      "BROWSER_CLOSE",
    );
    expect(`${bothFailed.stdout}${bothFailed.stderr}`).not.toContain(
      cookieSentinel,
    ...
    expect(`${closeOnly.stdout}${closeOnly.stderr}`).not.toContain(
      closeSentinel,
    );
  });
  ...
  it("submits the canonical bootstrap credential and proves provider callback completion", () => {
    ...
    expect(source).toContain(
      'page.locator("#login-name").fill(bootstrapLoginId)',
    );
    expect(source).toContain(
      'page.locator("#login-password").fill(previewPassword)',
    );
    expect(source).toContain(
    ...
    expect(
      source.indexOf("const credentialCompletion = Promise.allSettled(["),
    ).toBeLessThan(
      source.indexOf('page.locator("#login-name").fill(bootstrapLoginId)'),
    );
    expect(source).toContain("consoleCredentialCompletionFailureStage(");
    expect(source).toContain("await credentialCompletion");
    expect(source).toContain(
      "consoleCredentialFailureMarker(credentialFailureStage)",
    );
    expect(source).not.toContain(
      "Preview Console credential flow did not complete authenticated read-back",
    );
    ...
    expect(source).toContain(
      'consoleCredentialFailureMarker("COOKIE_CLEANUP")',
    );
