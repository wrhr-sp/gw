...
export const hotelFeatureGuides = {
  "hotel-owner-inquiry.lifecycle": {
    ...
    cautions: [
      "내부메모는 호텔 소유주에게 공개되지 않습니다.",
      "답변완료 후 7일 동안 보완요청이 없으면 자동으로 종료됩니다.",
      "종료된 문의는 30일 이내에만 재개할 수 있습니다.",
    ],
    featureKey: "hotel-owner-inquiry.lifecycle",
    permissions: [
      "호텔 소유주는 현재 연결된 자기 호텔 문의만 조회·작성할 수 있습니다.",
      "사내 임직원은 호텔배정과 문의별 조회·답변·담당 권한을 요청마다 확인합니다.",
    ],
    steps: [
      "문의처와 운영시간을 확인합니다.",
      "문의유형, 제목과 내용을 입력해 접수합니다.",
      "담당자는 공개답변과 내부메모를 구분해 기록합니다.",
      "소유주는 답변을 확인하고 보완요청하거나 종료합니다.",
      "종료 뒤 추가 확인이 필요하면 30일 이내에 재개합니다.",
    ],
    summary:
      "호텔 소유주 문의를 접수하고 담당 배정, 공개답변, 보완요청과 종료까지 관리합니다.",
    title: "호텔 소유주 문의",
    version: "1.0",
  },
  "hotel-daily-sales.ledger": {
    audience: [
      "일매출 조회권한과 현재 호텔배정 또는 소유주 연결이 있는 사용자",
    ],
    cautions: [
    ...
    permissions: [
      "조회·임시저장·확정·정정 권한과 현재 호텔배정 또는 소유주 연결을 요청마다 다시 확인합니다.",
    ],
    steps: [
      "업무일을 선택하고 매출구분·결제수단별 총매출, 할인, 환불을 입력합니다.",
      "임시저장 후 서버가 다시 계산한 순매출을 확인합니다.",
      "마감 증빙을 업로드하고 확정해 해당 업무일을 잠급니다.",
      "확정 뒤 오류가 있으면 정정 사유와 새 증빙으로 정정 버전을 등록합니다.",
    ],
    summary:
      "날짜별 매출을 임시저장하고 증빙과 함께 확정하며, 확정 뒤 변경은 정정이력으로 보존합니다.",
    title: "일매출 장부",
    version: "1.0",
  },
  ...
  "hotel-calendar.workspace": {
    ...
    permissions: [
      "달력 조회권한과 점검·보수 자료별 조회권한을 요청마다 다시 확인합니다.",
    ],
    steps: [
    ...
    summary:
      "권한 있는 호텔의 점검 마감과 보수 방문일정을 월간·주간 또는 날짜별 카드로 확인합니다.",
    title: "업무 달력",
    version: "1.0",
  },
  "hotel-repair.lifecycle": {
    audience: [
      "보수 조회·등록·방문·완료 권한과 현재 호텔배정이 있는 사내 임직원",
    ],
    cautions: [
    ...
    permissions: [
      "보수 자료 조회권한이 필요하며 등록·일정·완료는 각각의 동적 권한을 다시 확인합니다.",
    ],
    steps: [
      "대상과 하자증빙, 우선순위를 선택해 보수를 등록합니다.",
      "정확한 방문시간과 내부 수행자 또는 외부업체 한 곳을 등록합니다.",
      "작업결과와 완료증빙을 저장하고 프로세스 검토 뒤 최종완료합니다.",
    ],
    summary:
      "호텔 하자를 등록하고 여러 방문 작업과 최종완료, 후속 보수까지 관리합니다.",
    title: "하자·보수",
    version: "1.0",
  },
  ...
  "hotel-knowledge.bank": {
    audience: [
      "지식 조회권한과 현재 회사 또는 호텔 범위가 있는 사내 임직원, 하우스키핑, 호텔 소유주",
    ],
    cautions: [
      "고객 개인정보, 예약번호, credential은 작성하지 않습니다.",
      "재검토 필요로 표시된 자료는 현재 권장정보로 사용하지 않고 관리자에게 확인합니다.",
      "관련 운영이슈와 보수는 현재 조회권한을 통과한 자료만 표시됩니다.",
      "첨부 사진은 private 저장소와 악성파일 검역을 통과한 뒤에만 연결되며 외부 공개 URL을 만들지 않습니다.",
    ],
    featureKey: "hotel-knowledge.bank",
    permissions: [
      "회사 공통 또는 현재 배정된 호텔의 지식 조회권한을 요청마다 다시 확인합니다.",
      "작성·검토·게시·보관 권한은 분리되어 있으며 허용된 동작만 표시됩니다.",
      "첨부 열람 때도 활성 세션, 회사·호텔 범위, 현재 지식 조회권한을 다시 확인합니다.",
    ],
    steps: [
      "증상·상황·태그로 현장 지식을 검색하고 적용 범위와 상태를 확인합니다.",
      "먼저 확인할 사항, 가능한 대응, 금지 대응, 에스컬레이션 순서로 읽습니다.",
      "작성권한이 있으면 초안을 저장하고 검토를 요청합니다.",
      "초안 또는 재검토 필요 상태에서 현장 사진을 선택하고 검역 완료 후 지식에 연결합니다.",
      "검토·게시권한이 있는 담당자는 개인정보와 최신 운영기준을 확인한 뒤 게시합니다.",
      "도움 여부를 남기거나 잘못된 내용을 오류·수정 신고로 전달합니다.",
    ],
    summary:
      "회사 공통 및 배정된 호텔의 운영 지식을 안전하게 검색하고 검토·게시·평가합니다.",
    title: "운영 지식뱅크",
    version: "1.1",
  },
