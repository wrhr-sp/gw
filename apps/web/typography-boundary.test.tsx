...
describe("groupware typography baseline", () => {
  it("self-hosts Pretendard and routes all app text through typography tokens", () => {
    ...
    expect(realUsagePanels).toContain("import { boardTinymceInit } from \"./board-rich-editor-config\";");
    expect(realUsagePanels).toContain("init={boardTinymceInit}");
    expect(boardEditorConfig).toContain("font_family_formats: boardTinymceFontFamilyFormats");
    expect(boardEditorConfig).toContain("elementpath: false");
    expect(boardEditorConfig).toContain("height: 550");
    expect(boardEditorConfig).toContain("min_height: 550");
    expect(boardEditorConfig).toContain("font_size_formats: boardTinymceFontSizeFormats");
    expect(boardEditorConfig).toContain("content_style: boardTinymceContentStyle");
    expect(boardEditorConfig).not.toContain("font-family: Arial, sans-serif");
    expect(boardEditorConfig).toContain("export const boardTinymceFontSizeFormats = \"8pt 9pt 10pt 11pt 12pt 14pt 16pt 18pt 24pt 36pt\"");
    expect(boardEditorConfig).toContain("font-family: 'Malgun Gothic', '맑은 고딕', sans-serif; font-size: 10pt;");
    expect(boardEditorConfig).toContain("기본 글꼴=Pretendard Variable,Pretendard,'Malgun Gothic','맑은 고딕',sans-serif");
    expect(boardEditorConfig).toContain("프리텐다드=Pretendard Variable,Pretendard,sans-serif");
    expect(boardEditorConfig).toContain("본고딕=Noto Sans KR,Noto Sans CJK KR,Source Han Sans KR,sans-serif");
    expect(boardEditorConfig).toContain("수트=SUIT,sans-serif");
    expect(boardEditorConfig).toContain("스포카 한 산스=Spoqa Han Sans Neo,Spoqa Han Sans,sans-serif");
    expect(boardEditorConfig).toContain("나눔고딕=Nanum Gothic,NanumGothic,sans-serif");
    expect(boardEditorConfig).toContain("나눔스퀘어=NanumSquare,Nanum Square,sans-serif");
    expect(boardEditorConfig).toContain("맑은 고딕='Malgun Gothic','맑은 고딕',sans-serif");
    expect(boardEditorConfig).toContain("맑은 고딕 Light='Malgun Gothic Semilight','맑은 고딕 Semilight','Malgun Gothic','맑은 고딕',sans-serif");
    expect(boardEditorConfig).toContain("돋움=Dotum,'돋움',sans-serif");
    expect(boardEditorConfig).toContain("돋움체=DotumChe,'돋움체',monospace");
    expect(boardEditorConfig).toContain("굴림=Gulim,'굴림',sans-serif");
    expect(boardEditorConfig).toContain("굴림체=GulimChe,'굴림체',monospace");
    expect(boardEditorConfig).toContain("바탕=Batang,'바탕',serif");
    expect(boardEditorConfig).toContain("바탕체=BatangChe,'바탕체',monospace");
    expect(boardEditorConfig).toContain("궁서=Gungsuh,'궁서',serif");
    expect(boardEditorConfig).toContain("궁서체=GungsuhChe,'궁서체',monospace");
    expect(boardEditorConfig).toContain("Arial=Arial,Helvetica,sans-serif");
    expect(boardEditorConfig).toContain("Calibri=Calibri,Arial,sans-serif");
    expect(boardEditorConfig).toContain("Times New Roman=Times New Roman,Times,serif");
    expect(boardEditorConfig).toContain("Inter=Inter,Arial,sans-serif");
    expect(boardEditorConfig).toContain("Roboto=Roboto,Arial,sans-serif");
    expect(boardEditorConfig).toContain("Open Sans=Open Sans,Arial,sans-serif");
    expect(boardEditorConfig).toContain("IBM Plex Sans=IBM Plex Sans,Arial,sans-serif");
    expect(boardEditorConfig).toContain("Fira Code=Fira Code,Courier New,monospace");
  });
});
