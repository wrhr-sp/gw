...
describe("messenger page", () => {
  it("renders the board-like content title with conversation list and chat room columns plus a hidden new-message org popup", () => {
    ...
    expect(html).toContain("표시할 대상이 없습니다.");
    ...
    expect(pageSource).toContain("mailRecipientListResponseSchema");
    expect(pageSource).toContain("appRoutes.mail.recipients");
    expect(pageSource).toContain("memberIds: selectedContactIds");
    expect(pageSource).not.toContain("김민수");
    expect(pageSource).not.toContain("이서연");
    expect(pageSource).not.toContain("박지훈");
    expect(pageSource).not.toContain("최유진");
    expect(pageSource).not.toContain("정하늘");
    expect(pageSource).not.toContain("전사 공지문.docx");
    expect(pageSource).not.toContain("회의자료.pdf");
    expect(pageSource).not.toContain("근태현황.xlsx");
