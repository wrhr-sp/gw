...
test("PC 업무 달력은 월간·주간과 점검·보수 일정을 가로 넘침 없이 제공한다", async ({ mount, page }) => {
  ...
  const monthButton = component.getByRole("button", { name: "월간" });
  ...
  await expect(component.getByText("점검 마감").filter({ visible: true }).first()).toBeVisible();
  await expect(component.getByText("배관 점검").filter({ visible: true }).first()).toBeVisible();
...
test("390px 모바일은 축소 달력 대신 선택 날짜 현장업무 카드를 표시한다", async ({ mount, page }) => {
  ...
  await expect(component.getByText("Google 미연결")).toHaveCount(0);
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
  expect((await new AxeBuilder({ page }).include("section[aria-labelledby=calendar-title]").analyze()).violations).toEqual([]);
});
