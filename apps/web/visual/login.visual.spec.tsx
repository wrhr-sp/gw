...
test("비밀번호 정책 오류는 provider 제출 전에 새 비밀번호 입력으로 포커스를 이동한다", async ({ mount, page }) => {
  const reset = await mount(<PasswordResetCard />);
  for (const invalidPassword of ["", "Abc123!", "Abcd12😀", "PasswordOnly", "Abcd123 ", "Abcd123한"]) {
    ...
    await expect(reset.getByRole("alert")).toContainText("8자 이상이며 영문, 숫자, 기호를 각각 포함해야 합니다.");
...
test("서버 비밀번호 정책 오류는 오류 요약으로 포커스를 이동한다", async ({ mount, page }) => {
  const reset = await mount(
    <PasswordResetCard
      ...
      errorMessage="비밀번호는 8자 이상이며 영문, 숫자, 기호를 각각 포함해야 합니다."
      mode="form"
    />,
  );
