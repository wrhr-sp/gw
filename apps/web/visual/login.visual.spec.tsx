...
test("PC 로그인 기준 화면", async ({ mount, page }) => {
  ...
  const login = await mount(
    <HotelLoginCard authRequest="visual-auth-request" csrf={"c".repeat(43)} />,
  );
  await page.evaluate(() => document.fonts.ready);
  expect(
    await page.evaluate(() => document.fonts.check("16px Pretendard")),
  ).toBe(true);
  await expect(login.getByText("We’reHere")).toHaveCSS(
    "color",
    "rgb(25, 59, 87)",
  );
  await expect(login.getByLabel("아이디")).toHaveAttribute(
    "autocomplete",
    "username",
  );
  await expect(login.getByLabel("비밀번호")).toHaveAttribute(
    "autocomplete",
    "current-password",
  );
...
test("기존 reset cookie가 있어도 새 fragment를 URL 노출 없이 우선 교환한다", async ({
  mount,
  page,
}) => {
  await page.evaluate(() =>
    window.history.replaceState(
      null,
      "",
      "/#userID=subject-1&code=secret-code&orgID=org-1",
    ),
  );
  const requestPromise = page.waitForRequest((request) =>
    request.url().endsWith("/api/auth/password/exchange"),
  );
  ...
  expect(request.postData()).toBe(
    "code=secret-code&orgID=org-1&userID=subject-1",
  );
  await expect.poll(() => page.url()).not.toContain("secret-code");
});

test("기존 reset cookie와 malformed fragment는 서버 폐기 경로로 보낸다", async ({
  mount,
  page,
}) => {
  await page.evaluate(() =>
    window.history.replaceState(null, "", "/#unexpected=value"),
  );
  const requestPromise = page.waitForRequest((request) =>
    request.url().endsWith("/api/auth/password/exchange"),
  );
...
test("exchange network failure는 stale cookie form이 아닌 terminal 상태로 이동한다", async ({
  mount,
  page,
}) => {
  await page.evaluate(() =>
    window.history.replaceState(null, "", "/#userID=user-new&code=secret-code"),
  );
  await page.route("**/api/auth/password/exchange", async (route) => {
...
test("비밀번호 정책 오류는 provider 제출 전에 새 비밀번호 입력으로 포커스를 이동한다", async ({
  mount,
  page,
}) => {
  const reset = await mount(<PasswordResetCard />);
  for (const invalidPassword of [
    ...
    await expect(reset.getByRole("alert")).toContainText(
      "8자 이상이며 영문 소문자, 숫자, 기호를 각각 포함해야 합니다.",
    );
    await expect(
      reset.getByLabel("새 비밀번호", { exact: true }),
    ).toBeFocused();
    await expect(reset.getByLabel("새 비밀번호", { exact: true })).toHaveValue(
      invalidPassword,
    );
  }
  expect(
    await page.evaluate(() =>
      performance
        ...
        .some((entry) => entry.name.includes("/api/auth/password/set")),
    ),
  ).toBe(false);
  expect((await new AxeBuilder({ page }).analyze()).violations).toEqual([]);
});

test("비밀번호 불일치는 제출 전에 확인 필드와 연결한다", async ({
  mount,
  page,
}) => {
...
test("서버 비밀번호 정책 오류는 오류 요약으로 포커스를 이동한다", async ({
  mount,
  page,
}) => {
  ...
  await expect(
    ...
    "aria-describedby",
    "password-policy-description password-reset-error",
  );
  ...
  await expect(
    reset.getByLabel("새 비밀번호", { exact: true }),
  ).toHaveAttribute("aria-describedby", "password-policy-description");
  expect((await new AxeBuilder({ page }).analyze()).violations).toEqual([]);
});

test("최초 비밀번호 서버 오류는 입력에 연결되고 수정 시 해제된다", async ({
  mount,
  page,
}) => {
  await page.route("**/api/account/initial-password", async (route) =>
    route.fulfill({
      status: 400,
      ...
      body: JSON.stringify({
        ...
        error: {
          ...
          message: "비밀번호 정책을 확인해 주세요.",
          retryable: false,
          retryAfterSeconds: null,
          traceId: "50000000-0000-4000-8000-000000000077",
          fieldErrors: [
            {
              field: "newPassword",
  ...
  await form.getByLabel("새 비밀번호 확인").fill("Valid-password-123!");
  ...
  await password.fill("Another-valid-123!");
  await expect(form.getByRole("alert")).toHaveCount(0);
  await expect(form.getByText("새 비밀번호를 다시 입력해 주세요.")).toHaveCount(
    0,
  );
...
test("모바일 만료 링크는 실행 불가능한 폼을 숨긴다", async ({
  mount,
  page,
}) => {
  await page.setViewportSize({ width: 390, height: 844 });
  ...
  await expect(
    reset.getByRole("button", { name: "비밀번호 변경" }),
  ).toHaveCount(0);
  await expect(
    reset.getByRole("link", { name: "로그인 화면으로 돌아가기" }),
  ).toBeVisible();
  expect((await new AxeBuilder({ page }).analyze()).violations).toEqual([]);
});
