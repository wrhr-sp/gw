...
test("PC 사용자 생성 기준 화면", async ({ mount, page }) => {
  ...
  await expect(view.getByRole("button", { name: "사용자 생성", exact: true })).toHaveCSS("min-height", "44px");
...
test("하우스키핑 생성 POST는 두 호텔 배열만 전송", async ({ mount, page }) => {
  ...
  await view.getByLabel("로그인 아이디").fill("housekeeper99");
  ...
  await view.getByRole("button", { name: "사용자 생성", exact: true }).click();
...
test("서버 필드 오류는 해당 입력 수정 시 상단 오류와 함께 해제", async ({ mount, page }) => {
  ...
  await loginName.fill("duplicatelogin");
  ...
  await loginName.fill("availablelogin");
...
test("모바일 하우스키핑 복수 호텔 선택", async ({ mount, page }) => {
  ...
  expect((await new AxeBuilder({ page }).analyze()).violations).toEqual([]);
  const submit = view.getByRole("button", { name: "사용자 생성", exact: true });
