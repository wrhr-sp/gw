...
import { phase59GuideDocumentLinks } from "./app/uat/phase59-uat-config";
...
export function MenuPageContent({
  ...
  return (
    ...
    >
      ...
      <SurfaceSection title="UAT 체크 / 도움말 진입" description="전체 메뉴에서도 역할별 UAT entry 와 참고 문서를 다시 찾아, 홈에서 놓친 안내를 여기서 바로 이어 볼 수 있게 합니다.">
        <div className="grid-auto-compact">
          <article className="info-card">
            <Pill tone="accent">/uat</Pill>
            <h3>통합 UAT 체크 화면</h3>
            <p>역할별 entry, 시나리오 A~F, 역할별 검증표, 관리자 안내 흐름을 한 화면에서 다시 확인합니다.</p>
            <a href="/uat">/uat</a>
          </article>
          {phase59GuideDocumentLinks.slice(0, 3).map((item) => (
            <article key={item.docPath} className="info-card">
              <Pill>{item.route}</Pill>
              <h3>{item.title}</h3>
              <p>{item.summary}</p>
