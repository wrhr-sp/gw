...
describe("Phase 12 dashboard summary skeleton", () => {
  it("keeps dashboard focused on today's work, waiting items, status, and read-only entry points", () => {
    const html = renderToStaticMarkup(<DashboardPage />);

    expect(html).toContain("오늘 할 일");
    expect(html).toContain("승인/대기 요약");
    expect(html).toContain("근태/휴가 상태");
    expect(html).toContain("공지/문서 진입점");
    expect(html).toContain("운영 요약");
    expect(html.indexOf("오늘 할 일")).toBeLessThan(html.indexOf("승인/대기 요약"));
    expect(html.indexOf("승인/대기 요약")).toBeLessThan(html.indexOf("근태/휴가 상태"));
    expect(html.indexOf("근태/휴가 상태")).toBeLessThan(html.indexOf("공지/문서 진입점"));
    expect(html.indexOf("공지/문서 진입점")).toBeLessThan(html.indexOf("운영 요약"));
