    ...
    <section aria-labelledby="calendar-title" className="space-y-4">
      ...
      <div className="hidden gap-4 md:grid md:grid-cols-[minmax(0,1fr)_480px]">
        <div ref={calendarSurfaceRef} className="calendar-surface rounded-panel border border-border bg-surface p-4 shadow-sm">
          ...
          <FullCalendar ref={calendarRef} plugins={[dayGridPlugin, timeGridPlugin, interactionPlugin]} initialView="timeGridWeek" locale={koLocale} timeZone="Asia/Seoul" firstDay={0} height="auto" nowIndicator dayMaxEvents={4} allDaySlot={false} slotMinTime="06:00:00" slotMaxTime="24:00:00" events={fullCalendarEvents} headerToolbar={{ left: "prev,next today", center: "title", right: "" }} datesSet={(info) => { const from = localDate(info.startStr); const to = localDate(info.endStr); if (from !== range.from || to !== range.to) setRange({ from, to, timeZone: "Asia/Seoul" }); }} dateClick={(info) => setSelectedDate(info.dateStr.slice(0, 10))} eventClick={(info) => { setSelectedEventId(String(info.event.extendedProps.sourceId)); setSelectedDate(localDate(info.event.start?.toISOString() ?? info.event.startStr)); }}/>
        </div>
      ...
      {event ? (
        <div className="mt-5 space-y-3">
          ...
          <dl className="grid grid-cols-[100px_1fr] gap-y-3 text-sm">
            ...
            {event.type === "REPAIR_VISIT" ? <><dt className="text-muted">Google</dt><dd>Google 미연결</dd></> : null}
          </dl>
