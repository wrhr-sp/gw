      ...
      {!result.ok ? (
        ...
        <>
          <PageHeader
            actions={
              <StatusBadge
                tone={hotelStatusPresentation[result.hotel.status].tone}
              >
                {hotelStatusPresentation[result.hotel.status].label}
              </StatusBadge>
            }
          ...
          >
            <span
              aria-current="page"
              className="inline-flex min-h-11 items-center border-b-2 border-primary px-1 text-sm font-semibold text-primary"
            >
              기본정보
            </span>
            <a
          ...
          <section className="rounded-panel border border-border bg-surface p-5 md:p-6">
            ...
            <dl className="mt-5 grid gap-x-8 gap-y-5 sm:grid-cols-2">
              <div>
                <dt className="text-xs font-semibold text-muted">호텔코드</dt>
                <dd className="mt-1 text-sm text-text">
                  {result.hotel.branchCode}
                </dd>
              </div>
              <div>
                <dt className="text-xs font-semibold text-muted">호텔명</dt>
                <dd className="mt-1 text-sm text-text">{result.hotel.name}</dd>
              </div>
              <div className="sm:col-span-2">
                <dt className="text-xs font-semibold text-muted">도로명주소</dt>
                <dd className="mt-1 text-sm text-text">
                  {result.hotel.roadAddress}
                </dd>
              </div>
              <div className="sm:col-span-2">
                <dt className="text-xs font-semibold text-muted">상세주소</dt>
                <dd className="mt-1 text-sm text-text">
                  {result.hotel.detailAddress || "없음"}
                </dd>
              </div>
              <div>
                <dt className="text-xs font-semibold text-muted">대표연락처</dt>
                <dd className="mt-1 text-sm text-text">
                  {result.hotel.representativePhone}
                </dd>
              </div>
              <div>
                ...
                <dd className="mt-1 text-sm text-text">
                  {result.hotel.contractStartDate} ~{" "}
                  {result.hotel.contractEndDate}
                </dd>
              </div>
              <div>
                <dt className="text-xs font-semibold text-muted">
              ...
              <div>
                ...
                <dd className="mt-1 text-sm text-text">
                  {new Intl.DateTimeFormat("ko-KR", {
                    ...
                    timeStyle: "short",
                    timeZone: "Asia/Seoul",
                  }).format(new Date(result.hotel.updatedAt))}
                </dd>
              </div>
            </dl>
          </section>
          <RelationshipManagementPanel
            ...
            hotelVersion={result.hotel.version}
