...
export const roleScenarioCards = [
  {
    ...
    statusChecks: [
      ...
      "offline: 네트워크 불안정 시 `/offline` 복구 순서가 따로 보여야 한다.",
    ],
  },
  {
    role: "승인자 / 팀장",
    ...
    statusChecks: [
      ...
      "offline: 승인 처리 불가 시 재시도/복구 순서가 안내돼야 한다.",
    ],
  },
  {
    role: "경영업무 담당자",
  ...
  {
    role: "운영자 / 감사",
    startRoute: "/admin 또는 /admin/audit-logs",
    journey: ["/admin", "/admin/users", "/admin/policies", "/admin/audit-logs", "/offline"],
    actions: [
      "일반 host 와 admin host 경계가 섞이지 않는지 먼저 확인한다.",
      "사용자/정책/감사 진입이 역할별로 다른지 확인한다.",
      "audit preview 가 read-only/masked 상태를 지키는지 확인한다.",
    ],
    happyPath: [
      "`AUDITOR`, `HR_ADMIN`, `COMPANY_ADMIN` 차이가 같은 관리자 묶음처럼 보이지 않는다.",
      "`/admin/audit-logs` 는 masked preview 와 company boundary 를 유지한다.",
      "admin host `/offline` 복구 범위가 일반 host 와 섞이지 않는다.",
    ],
    statusChecks: [
      "forbidden: 감사 권한 없는 역할이 audit preview 를 보면 blocker 다.",
      "error: route/API 실패는 empty 나 권한 부족과 분리해 적는다.",
      "offline: 관리자 복구 순서에 일반 직원용 route 를 섞지 않는다.",
    ],
  },
] as const;
...
export const issueLogTemplate = [
  ...
  "역할: 직원 | 승인자 | 경영업무 담당자 | 운영자/감사",
  "재현 route: 예) /dashboard → /management → /payroll",
  "기대 결과: 무엇이 보여야 하는가",
  "실제 결과: 무엇이 달랐는가",
  "상태 분류: happy path | forbidden | empty | error | offline | approval-needed",
...
export const facilitatorScript = [
  ...
  "3분: 직원 레인(`/dashboard` 중심)과 경영업무/운영 레인(`/management`, `/admin*`)을 섞지 말라고 설명한다.",
  "4분: happy path 와 forbidden/empty/error/offline 을 다른 상태로 기록해야 한다고 예시를 든다.",
  "5~8분: 역할별 추천 시나리오를 따라 직접 클릭하게 하고, 막히는 지점은 즉시 이슈 템플릿에 적게 한다.",
