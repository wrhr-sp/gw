...
describe("Phase 41 collaboration adoption fit-gap", () => {
  it("keeps board write title as plain page-title sized text without a pill/card wrapper", () => {
    ...
    expect(globalCss).toContain("--board-write-notice-grid-template-columns: 104px max-content max-content max-content max-content minmax(0, 1fr) max-content;");
    ...
    expect(globalCss).toContain("grid-column: 7;");
  ...
  it("renders board routes with practical board flow, role guidance, and forged guard language", async () => {
    ...
    expect(boardListHtml).toContain('aria-label="게시판 더보기 메뉴 열기"');
    expect(boardListHtml).toContain("가이드");
    expect(boardListHtml).toContain("통합설정");
    ...
    expect(boardListHtml).not.toContain("공지 보기");
