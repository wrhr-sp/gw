...
describe("org/employees/admin boundaries", () => {
  ...
  it("keeps management support employee management list-first and opens creation in the right detail panel", async () => {
    ...
    expect(clientSource).toContain("departmentDutiesResponseSchema");
    expect(clientSource).toContain("getSelectedDepartmentDutyNames");
    expect(clientSource).toContain("toggleDepartmentDutySelection");
    ...
    expect(clientSource).toContain("hydrateCreatePanelFromSelected(item)");
    expect(clientSource).toContain("employee-detail-copied-create-form");
    expect(clientSource).toContain("사원 상세패널 생성 입력화면 복사본");
    expect(clientSource).toContain("사원 상세패널 입력화면 작업");
    expect(clientSource).not.toContain("activeDetailPanelTab === \"profile\"");
    expect(clientSource).not.toContain("activeDetailPanelTab === \"organization\"");
    expect(clientSource).not.toContain("activeDetailPanelTab === \"account\"");
    expect(clientSource).not.toContain("activeDetailPanelTab === \"security\"");
  ...
  it("applies the employee create right-panel standard to admin organization detail panels", async () => {
    ...
    expect(clientSource).toContain("aria-label=\"등록/수정 패널\"");
    expect(clientSource).toContain("aria-label=\"기능페이지 목록\"");
    expect(clientSource).toContain("role=\"navigation\"");
    expect(clientSource).toContain("+ 추가");
    expect(clientSource).toContain("aria-label=\"목록 전체 선택\"");
    ...
    expect(pageSource.match(/조직정보/g)?.length).toBe(1);
