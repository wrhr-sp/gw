...
- **도담** (`gwplanner`, 기획봇): 계획, 우선순위, 선택지 정리
- **이룸** (`gwbuilder`, 구현봇): 구현, 파일 수정, 코드 작성
- **바름** (`gwreviewer`, 리뷰봇): 리뷰, 누락 점검, 품질 확인
- **해봄** (`gwtester`, 테스트봇): 테스트, 검증, 실패/성공 확인
- **지킴** (`gwops`, 운영봇): 배포·운영 위험 점검
- **다온** (`gwdocs`, 문서봇): 문서화, 쉬운 설명 정리
