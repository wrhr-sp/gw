...
- 외부 달력 서비스 UI·iframe·provider ID·동기화 구현정보를 제품에 노출하거나 사용하지 않는다.

### 운영 지식뱅크

```text
PageHeader → 검색 → 지식 목록 → 안전 대응 순서 상세 → PrivateAttachmentPanel → 피드백
```

- PC는 목록·상세 2열을 사용하고 모바일은 표를 축소하지 않고 안전 대응 순서와 주요 행동을 단일 열 카드로 배치한다.
- 상세는 `먼저 확인할 사항 → 가능한 대응 → 하지 말아야 할 대응 → 에스컬레이션` 순서를 유지한다.
- 회사 공통·호텔 전용 범위와 게시·재검토 상태를 색상뿐 아니라 문구로 표시한다.
- 작성 버튼과 범위 selector는 회사 `canCreate`와 호텔별 `permissions.canCreate`를 각각 적용하고, 읽기 전용 호텔이나 거부된 회사 공통 범위를 작성 옵션에 노출하지 않는다.
- 첨부는 초안·재검토 필요 상태에서 작성권한자만 추가하며, private R2 저장과 악성파일 검역이 끝나기 전에는 지식에 연결하거나 열람 링크를 표시하지 않는다.
- 수정·검토요청·게시·재검토·보관·첨부 버튼은 전역 메뉴 capability가 아니라 현재 상세 응답의 resource-scoped `actions`만 사용한다.
- 같은 지식·파일 재시도는 init·complete·attachment-link의 exact body/key와 upload/ETag를 연결 성공까지 유지하고, 응답 유실·재로그인 뒤에도 같은 버튼으로 동일 결과를 replay해야 한다.
- 첨부 열람은 public object URL이 아니라 same-origin exact view route를 사용하고, 서버가 활성 세션·회사·호텔·현재 지식권한을 다시 검증한다.
- 390px에서는 파일선택과 `사진 업로드·검역`을 전체 폭 44px 이상 행동으로 배치하고 검역 진행·실패·지연 상태를 `aria-live` 문구로 알린다.
