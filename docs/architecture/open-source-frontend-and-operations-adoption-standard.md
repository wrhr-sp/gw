...
2. 사원정보관리 목록 표준화
   - `@tanstack/react-table`을 사원정보관리 목록의 headless table 엔진으로 도입한다.
   - 기존 등록 팝업, 우측 상세패널, 탭 구조, 저장 후 재조회 흐름을 유지한다.
   - 렌더링과 스타일은 기존 `employee-management-table`과 그룹웨어 토큰을 유지한다.

3. B안 Docker 참고 도구 격리 실행 기준
   - `ops/docker/open-source-reference.compose.yaml`을 사용한다.
   - Appsmith/NocoDB는 localhost 참고 도구로만 실행한다.
   - 실DB, secret, production/custom domain, 개인정보 연결은 별도 승인 전에는 금지한다.
...
다음 작업은 공통 인터랙션 컴포넌트 정리다. 사원정보관리에서 이미 쓰는 등록 dialog와
상세패널 tabs를 기준으로 Radix/shadcn 접근성 패턴을 비교하고, 필요한 부분만 그룹웨어
토큰 기반 내부 컴포넌트로 재작성한다. Tailwind나 shadcn 기본 theme는 도입하지 않는다.
