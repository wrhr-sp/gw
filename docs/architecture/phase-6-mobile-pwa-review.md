...
2. 모바일에서 자주 쓰는 출퇴근/휴가/전자결재 CTA 는 카드형/큰 버튼 Production-ready (실구현) 로 재배치했고, disabled 상태와 설명 문구로 아직 Production-ready (실구현) 단계임을 유지했다.
...
6. icon 은 Production-ready (실구현) SVG 를 추가했지만 실제 브랜드/스토어용 자산이 아니라는 점을 유지한다.
