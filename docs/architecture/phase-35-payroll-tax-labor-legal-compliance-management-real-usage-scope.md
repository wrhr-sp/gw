...
2. `/payroll` 과 `/payroll/me` 는 same-origin API와 권한 테스트가 이미 있지만,
   여전히 preview/Production-ready (실구현)/payroll foundation 톤이 강하다.
...
- `apps/web/app/payroll/page.tsx`
  - 급여 프로필 Production-ready (실구현), 급여 기간 상태, 직원용 명세서 초안, 연결 API 목록을 route 에서 바로 보여 준다.
...
- legal 은 계약 검토/갱신/분쟁 intake 가 visibility 경계와 함께 Production-ready (실구현) 로 존재한다.
...
- `/work-items/tax`
  - 세무 category, 제출 상태, HQ review/deadline Production-ready (실구현)
- `/work-items/labor`
  - 계약/수당/고충/징계 등 labor metadata Production-ready (실구현)
- `/work-items/legal`
  - 계약 검토/갱신/분쟁 후속 Production-ready (실구현)
- `/admin/audit-logs`
...
- 원천세 Production-ready (실구현) 와 세무 마감 준비를 같은 모듈 책임처럼 적지 않는다.

### 결정 C. tax/labor/legal 은 공통 `work item` Production-ready (실구현) 위에 올라간 관리자 업무로 적는다.
...
| 세무 | `/work-items/tax`, `/api/work-items?module=tax`, `/api/work-item-deadlines`, review Production-ready (실구현) | 홈택스 직접 신고, 외부 세무사 계정, 실원문 전송 없음 | branch scope vs company scope 세무 경계 문서화와 UAT 순서 고정 |
...
| 법무 | `/work-items/legal`, 계약 검토/갱신/분쟁 intake Production-ready (실구현), review Production-ready (실구현) | 실계약 원문 저장, 외부 변호사/보험 연동, 실제 분쟁 제출 없음 | 현재 Production-ready (실구현) 3건과 visibility 경계를 과장 없이 고정 |
...
| 권한/범위 | payroll role split, tax company/branch split, labor restricted, legal visibility, audit read 경계 | 왜 막히는지 더 즉시 읽히는 UX 보강 필요 | forbidden/company scope/branch scope/self-only/Production-ready (실구현) 제한 문구 통일 |
...
4. `/payroll`
   - 급여 프로필 Production-ready (실구현), 기간 상태, 직원 명세서 초안 분리가 읽히는지 본다.
5. `/payroll/me`
...
| 직원용 급여명세서 | `/payroll/me` | `payroll.payslip.read_self` | self-only 명세서 초안과 정정 안내를 본다 | preview 상태, self-only, correction guidance 가 읽힘 | 자기 권한이 아니면 403 | 원천세/4대보험 Production-ready (실구현), 실엔진 미연동 |
| 세무 업무 | `/work-items/tax` | `work_item.read` + scope | tax milestone, API route, roleScope 를 본다 | branch 제출 상태와 HQ review Production-ready (실구현) 이 읽힘 | branch/company scope 가 다르면 차단 | 직접 신고/외부 전송 없음 |
...
| 법무 업무 | `/work-items/legal` | `work_item.read` + scope | 계약 검토/갱신/분쟁 후속 문구를 본다 | legal category 와 approval gate 문구가 읽힘 | company scope 민감 계약은 차단 | Production-ready (실구현) 3건 중심, 실계약 원문 없음 |
...
- payroll/work-items/audit 중 fixture/Production-ready (실구현) 와 운영 read fallback 경계 구분
