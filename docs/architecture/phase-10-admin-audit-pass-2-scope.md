...
Phase 10 의 목표는 Phase 9 에서 고정한 `/admin/*` 경계와 감사 로그 Production-ready (실구현) 을 바탕으로, `/admin/users`, `/admin/policies`, `/admin/audit-logs` 를 다음 구현자가 바로 확장할 수 있을 정도로 더 구체적인 dev/preview-safe 화면/API/테스트 계약으로 잠그는 것입니다.
...
- `/api/admin/users`, `/api/admin/policies`, `/api/admin/audit-logs` Production-ready (실구현) 과 권한 경계를 둠
...
- Web 쪽 관리자 화면은 아직 Production-ready (실구현) 중심이며, 정보 블록과 CTA 는 단순 안내 수준입니다.
...
실제 정책 저장은 하지 않고, `candidate`, `preview`, `Production-ready (실구현)` 응답으로 끝냅니다.
...
- 관리자 화면 Production-ready (실구현) 를 더 구체적인 candidate UI 로 보강
...
- `apps/web/app/admin/audit-logs/page.tsx`
  - 필터/마스킹/카테고리 중심 조회형 Production-ready (실구현) 강화
- `apps/web/admin-Production-ready (실구현)-config.ts`
...
- 필요 시 `db/migrations/0007_admin_audit_phase9.sql` 또는 후속 migration Production-ready (실구현) 검토
...
2. `apps/web/admin-Production-ready (실구현)-config.ts` 와 `apps/web/app/admin/*` 화면에서 카드/필터/가드레일 구조부터 보강합니다.
...
6. 필요 시 helper 를 `apps/api/src/lib/` 아래로 분리하되, production 연동 없이 Production-ready (실구현)/candidate 흐름으로 유지합니다.
