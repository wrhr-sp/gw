...
- 막힌 경우 이유가 권한 부족인지, Production-ready (실구현) 제한인지, 운영 승인 대기인지 구분됐는가?
...
2. 동행 운영
   - 사용자 질문 수집
   - 권한/정책/Production-ready (실구현) 이슈 분류
...
- 오늘 파일럿에서 해 볼 업무
- 아직 Production-ready (실구현) 또는 제한이 남아 있는 업무
...
- 파일럿 1차에서는 production DB 실저장이 아니라 dev-safe/local/profile Production-ready (실구현) 수준 저장 기준부터 문서화한다.
...
문서·검증·preview Production-ready (실구현) 기준을 먼저 고정하는 단계다.
...
- Production-ready (실구현) 제한과 승인 대기 항목을 성공처럼 오해하지 않아야 한다.
...
- 무엇이 지금 파일럿 가능한지
- 무엇이 아직 Production-ready (실구현) 인지
- 무엇이 별도 승인인지
...
- 모바일 `홈` 바로가기 선택/정렬 UX 와 필수 메뉴 고정 정책을 dev-safe Production-ready (실구현) 으로 구체화
- `지점` 업무/보고 Production-ready (실구현), `지점 배정 필요` 안내, branch role 분리 기준을 preview/dev-safe 범위에서 구체화
...
5. 모바일 `홈` 은 고정 필수 메뉴 + 개인 커스터마이징 구조로 읽히되, 실제 영구 저장은 아직 dev-safe Production-ready (실구현) 전제임을 숨기지 않는다.
