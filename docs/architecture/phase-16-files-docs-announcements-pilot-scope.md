...
Phase 16의 목표는 이미 있는 게시판/공지/문서함/R2 skeleton과
...
- `/boards`, `/boards/[boardId]`, `/posts/[postId]`, `/documents` placeholder 화면
- board/document shared contract 와 API skeleton
- R2 연계를 위한 private-by-default 원칙, metadata 우선 원칙, binding-aware skeleton 방향
...
- 게시판/문서 API skeleton, 문서 공간 권한, metadata 비노출, forged/private 접근 차단 테스트는 이미 존재합니다.
...
- `/boards`, `/boards/board_notice`, `/boards/board_general`, `/posts/board_post_board_notice_employee_employee`, `/documents` 흐름을 preview/local 문맥에서 읽기 중심 placeholder 로 따라갈 수 있습니다.
...
- 홈/로그인/대시보드 입구 흐름
- 근태/휴가/결재 기본 업무 흐름
- 게시판/공지 목록, 게시글 상세, 댓글 skeleton 흐름
- 문서함/문서 공간/첨부 metadata skeleton 흐름
...
- mock/local-safe adapter 또는 metadata skeleton 기준으로 설명 가능함
...
### 결정 E. blocked/empty/error 와 placeholder 설명은 게시판/문서 흐름에도 같은 기준으로 확장한다.
...
- 첨부 업로드/다운로드 미연결은 "placeholder/dev-safe 제한" 축으로 설명
...
- `/boards`, `/boards/[boardId]`, `/posts/[postId]`, `/documents` 의 placeholder/권한/안내 문구 보강
- board/document shared contract 와 API skeleton의 설명 정렬
