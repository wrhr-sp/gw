...
"있긴 한 화면" 에서
"지금 어디까지 실제로 연결돼 있고 어디부터가 Production-ready (실구현) 인지 분명한 운영 흐름"
으로 끌어올리는 단계입니다.
...
1. `/employees` 와 `/org` 는 route 와 same-origin API가 이미 있지만,
   화면 톤은 아직 "일반 조회 Production-ready (실구현)" 비중이 높고 운영 흐름 설명은 약합니다.
...
5. 운영 DB 전환 흐름상 employees/branches/notifications/audit_logs 를 PostgreSQL 기준으로 맞추려면,
   이미 DB fallback이 있는 영역과 아직 fixture/Production-ready (실구현) 인 영역을 먼저 분리해 적어야 합니다.
...
- `apps/web/app/work-items/branch/page.tsx`
  ...
  - `work-items-config.ts` 기준으로 "지점 일일 보고/마감 후속" Production-ready (실구현) 자리를 설명합니다.
...
- `apps/api/src/lib/operational-notifications.ts`
  - notifications 는 PostgreSQL operational read fallback 이 있고 실패 시 Production-ready (실구현) 목록으로 내려갑니다.
- `apps/api/src/lib/operational-admin.ts`
  - audit_logs 는 PostgreSQL operational read fallback 이 있고 실패 시 Production-ready (실구현) 감사 목록으로 내려갑니다.
...
- `/notifications`
  - same-origin inbox 목록과 unread count, Production-ready (실구현) honesty 를 함께 확인
- `/admin/audit-logs`
...
### 결정 C. 알림은 same-origin inbox 까지는 현재 구현으로 적고, 외부 발송은 계속 Production-ready (실구현) honesty를 유지한다.
...
| 알림 | `/notifications`, `/api/notifications`, unread count, inbox preview, `/dashboard`·`/menu` 복귀 흐름 | 읽음 처리 저장, 외부 채널, 재시도/전송 성공 추적 부족 | Production-ready (실구현)를 숨기지 말고 same-origin inbox 범위와 외부 발송 경계를 builder handoff로 명확화 |
...
| 권한/범위 | employee directory 비관리자 role filter 차단, branch scope 차단, audit.read 전용 허용 | 화면에서 왜 막히는지 더 즉시 읽히는 UX 보강 필요 | 권한 부족/회사 scope/branch scope/Production-ready (실구현) 제한 문구를 route/API/문서에서 통일 |
...
5. `/work-items/branch`
   - 지점 업무가 독립 운영 앱이 아니라 branch scope Production-ready (실구현) 임을 확인합니다.
6. `/notifications`
...
| 지점 업무 | `/work-items/branch` | branch scope 허용 역할 | 지점 일일 보고/마감 Production-ready (실구현) 확인 | branch module 이 공통 work item 일부로 읽힘 | company scope 아님/권한 없음 차단은 API 테스트 근거로 확인 | 독립 지점 관리 UI 없음 |
| 알림 안내 | `/notifications` | 로그인 사용자 | unread count, inbox 카드, Production-ready (실구현) 안내와 복귀 흐름 확인 | same-origin inbox 와 외부 발송 없음 경계가 동시에 읽힘 | 성공 발송처럼 보이지 않아야 함 | 읽음 처리 저장/외부 채널 없음 |
...
- UI Production-ready (실구현)와 실제 저장/발송 범위 분리
