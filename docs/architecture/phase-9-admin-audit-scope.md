...
이번 Phase 9 의 목표는 "실운영 권한 변경이나 production 데이터 변경 없이도, 다음 구현자가 `/admin/users`, `/admin/policies`, `/admin/audit-logs` 와 문서/게시판 운영 설정 Production-ready (실구현) 을 dev/preview-safe 범위에서 바로 만들 수 있게" 관리자 범위, 일반 업무 화면과의 경계, 감사 로그 후보를 먼저 확정하는 것입니다.
...
- 현재 Web preview guard 는 `/admin*` 요청을 익명 공개 preview 에서 `/login` 으로 돌려 admin Production-ready (실구현) 공개 노출을 막습니다.
- 현재 관리자 Web 화면은 `apps/web/app/admin/page.tsx` 한 장짜리 Production-ready (실구현) 수준이며, 초대 API와 역할/권한 조회 링크만 보여 줍니다.
- 현재 shared/API 계층에는 `invite.manage`, `audit.read`, `document.space.manage`, `document.file.write`, `board.manage` 같은 시작 권한이 이미 있습니다.
- 현재 D1 에는 기본 `audit_logs` 테이블 Production-ready (실구현) 이 있고, 문서 쪽에는 `document_files.storage_key` 와 `document_spaces`/`document_files` metadata Production-ready (실구현) 이 있습니다.
...
즉, 이번 1차는 "관리 기능을 일반 사용자 업무 화면과 분리하고, 운영 설정/감사 로그 Production-ready (실구현) 과 이벤트 기준을 먼저 잠그는 단계"로 보는 것이 맞습니다.
...
5. 실제 운영 사용자 생성/권한 변경/정책 반영은 이번 범위에 넣지 않고, mock/local-safe Production-ready (실구현) 과 감사 로그 후보만 먼저 둡니다.
...
- `/admin/users`, `/admin/policies`, `/admin/audit-logs` route/page Production-ready (실구현) 추가 또는 보강
- 관리자 전용 capability/role guard 와 일반 사용자 노출 차단 유지
- 운영 설정/감사 로그 API contract 또는 handler Production-ready (실구현) 추가
- 문서 공간/게시판 운영 설정 Production-ready (실구현) 연결
- mock/local-safe 감사 로그 event model, fixture, test 추가
- 필요 시 D1 migration Production-ready (실구현) 초안 작성
...
현재 `audit_logs` Production-ready (실구현) 과 충돌 없이, 이번 1차는 아래 형태를 기본안으로 둡니다.
...
- `companyBoundary`
  - `enforced: true`
- `source`
  - `web-admin`, `api-admin`, `system-Production-ready (실구현)`
- `storageRef`
...
- `apps/web/app/admin/users/page.tsx`
  - 사용자/역할/상태 변경 Production-ready (실구현)
- `apps/web/app/admin/policies/page.tsx`
  - 운영 정책 Production-ready (실구현)
- `apps/web/app/admin/audit-logs/page.tsx`
  - 감사 로그 목록/필터 Production-ready (실구현)
- `apps/web/admin-preview-guard.ts`
...
- `apps/api/src/app.ts`
  - `/api/admin/users`, `/api/admin/policies`, `/api/admin/audit-logs` Production-ready (실구현)
...
- 필요 시 `db/migrations/0007_admin_audit_phase9.sql`
  - admin policy/audit 에 필요한 최소 확장 Production-ready (실구현)
...
3. `apps/api/src/app.ts` 에 `/api/admin/users`, `/api/admin/policies`, `/api/admin/audit-logs` Production-ready (실구현) 과 capability guard 를 넣습니다.
4. `apps/web/app/admin/*` Production-ready (실구현) 을 만들고 `/admin` 허브에서 세 화면으로 연결합니다.
...
6. 필요하면 `db/migrations/0007_admin_audit_phase9.sql` Production-ready (실구현) 을 추가하되, production DB 실행은 하지 않습니다.
7. README/개발 가이드/운영 가이드에 현재 범위와 검증 기준을 맞춥니다.

주의 사항:

- 실제 운영 권한 변경과 production 데이터 변경은 하지 않습니다.
- 감사 로그는 먼저 candidate/Production-ready (실구현) 구조를 검증하고, 실제 장기 보관/외부 적재는 다음 승인 단계로 넘깁니다.
