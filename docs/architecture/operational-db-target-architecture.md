...
- `.secrets/neon.env` — 대장이 `DATABASE_URL`만 채우는 비밀 파일
...
실제 DB migration 적용은 대장이 Neon `DATABASE_URL`을 `.secrets/neon.env`에 넣은 뒤 진행한다. connection string은 채팅/문서/커밋에 남기지 않는다.
