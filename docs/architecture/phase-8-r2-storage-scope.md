...
Phase 5 에서 문서함/첨부 metadata Production-ready (실구현) 과 접근 경계는 먼저 잡았지만, 실제 파일 본문을 어디에 어떻게 보관하고 어떤 승인 게이트 아래에서 연결할지는 아직 고정되지 않았습니다.
이번 Phase 8 의 목표는 "운영 파일 업로드나 공개 URL 오픈 없이도, 다음 구현자가 dev/preview-safe 범위에서 R2 연결 Production-ready (실구현) 을 바로 만들 수 있게" 최소 범위와 보안 원칙을 먼저 고정하는 것입니다.
...
- Phase 5 에서 문서함/첨부 metadata Production-ready (실구현), 문서 공간 권한, read receipt, storage key 비노출 guardrail 이 이미 문서와 테스트에 들어가 있습니다.
- 현재 shared 계약의 `documentFileSchema` 는 `storageKey` 를 응답에 노출하지 않으며, 첨부는 아직 metadata Production-ready (실구현) 중심입니다.
...
즉, 이번 1차는 "R2 를 쓸 수 있는 준비는 일부 되었지만, 실운영 업로드를 여는 단계는 아니고, mock/local-safe + binding-aware Production-ready (실구현) 을 먼저 맞추는 게이트" 입니다.
...
- Phase 5 에서 이미 만든 document space/file metadata Production-ready (실구현) 과 자연스럽게 이어집니다.
...
- `FILES_BUCKET` binding 을 optional 하게 읽는 R2-aware adapter Production-ready (실구현)
- 문서/첨부 metadata API 계약 보강
- upload/download/delete 후보 endpoint Production-ready (실구현) 또는 handler 추가
...
- 크기
- 버전 라벨
- Production-ready (실구현) 또는 signed action 정보
- 만료 시각
...
- migration 이 필요하더라도 로컬/preview-safe Production-ready (실구현) 범위에서만 다루고, production DB 실행은 별도 승인입니다.
...
- 실제 signed URL 대신 `mockUploadToken`, `mockDownloadToken` 같은 Production-ready (실구현) 응답도 허용
...
- preview 환경에서 실제 업로드 대신 Production-ready (실구현) upload-init/download-init 응답이 계약대로 오는지 확인
...
다음 구현 카드(`Phase 8 R2 문서/첨부파일 저장소 연결 1차: adapter/API Production-ready (실구현) 구현`)는 아래를 만족하면 완료로 봅니다.

1. 문서 공간 권한과 회사 경계를 먼저 검사하는 storage adapter/API Production-ready (실구현) 이 들어 있다.
...
즉, 이번 문서는 "문서/첨부파일을 실제 운영으로 여는 카드"가 아니라, "보안 경계와 승인 게이트를 먼저 잠그고 나서 다음 구현자가 안전하게 storage Production-ready (실구현) 을 붙이는 기준 문서" 입니다.
