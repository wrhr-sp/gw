...
쉽게 말하면 이번 단계는
예전 Phase 5/16에서 만든 게시판·문서 skeleton을
"설명용 placeholder"에서
...
- COMPANY_ADMIN 은
  ...
  - upload-init / upload-complete / download-init / delete placeholder action 이 존재함
...
- 게시글 상세 화면에 실제 본문/댓글 스레드/읽음 수 요약이 카드 중심 placeholder 설명에 가깝게 남아 있음
- 문서함은 목록/metadata는 있으나 문서 상세 route, 첨부 후속 액션, 버전 비교 UX가 아직 약함
- upload/download/delete 는 API placeholder 동작이 있지만,
...
| 게시판 상세 | `/boards/board_notice`, `/boards/board_general` route 진입, preview 생성, 현재 세션 guard 확인 | boardId별 실제 글 목록/작성/상태 반영 UX는 placeholder 비중 큼 | notice-only 대 general 책임을 실제 데이터/CTA 기준으로 강화 |
...
| 저장소/R2 경계 | metadata 생성, upload/download/delete placeholder action, raw storage 비노출 | 실제 운영 업로드/외부 공유처럼 오해될 수 있음 | PostgreSQL metadata 우선 + R2 원본 유지 경계를 더 분명히 |
...
- space 목록
- file metadata
- upload/download/delete placeholder action 설명
- private space 차단
- raw storage 비노출 유지
...
- 게시판/문서함이 same-origin API 응답을 실제로 읽으면서도 placeholder honesty를 잃지 않는지
