...
- `pnpm check` → 통과
- `pnpm --filter @gw/web build` → 통과
- `pnpm --filter @gw/web build:cf` → `/admin/users` prerender 중 `.next/server/app/admin/users/page.js` 누락으로 실패
...
- 저장소 코드에서는 same-origin `/api/health`, `/api/me` 가 다시 연결됐지만, 운영 관점에서는 `build:cf` blocker 를 풀기 전까지 공개 preview smoke 완료로 적으면 안 됩니다.
...
- 현재 URL handoff 문서에는 "저장소 코드는 same-origin 브리지까지 들어왔지만, `pnpm --filter @gw/web build:cf` blocker 때문에 공개 preview 재검증은 아직"이라는 메모를 같이 남겨야 합니다.
