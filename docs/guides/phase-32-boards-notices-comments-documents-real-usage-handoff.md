...
아직 부족한 것:
- 화면이 실제 UAT 흐름보다 설명형 placeholder 비중이 아직 큼
...
- COMPANY_ADMIN
  ...
  - upload/download/delete placeholder action 존재
...
- `/boards` 와 `/documents` 가 실제 API 응답을 읽더라도 여전히 placeholder honesty 를 유지하는지
...
6. `/posts/board_post_board_general_employee_employee` — 댓글 preview 생성/읽음 확인 등록/forged 차단 확인
7. `/documents` — metadata preview 생성/문서 읽음 확인/private·missing space 차단 확인
8. `/admin/policies` — 운영 정책 화면과 일반 협업 화면 경계 비교

## 10. 이번 문서화에서 일부러 분리해 적어야 하는 것
- 지금 바로 체험 가능한 route
- API 테스트로 이미 붙잡힌 happy path
- 화면은 있으나 placeholder 비중이 큰 route
