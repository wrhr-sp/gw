...
- 막힌 이유가 권한 문제인지, placeholder 제한인지, 승인 대기인지 구분됐는가?
...
- 이번 파일럿 문서 단계에서는 production DB 영구 저장이 아니라 dev-safe/local/profile skeleton 저장 전제를 먼저 문서화한다.
...
이번에는 preview/dev-safe skeleton 기준을 고정하는 단계다.
...
- `지점` 업무·보고 placeholder 와 `지점 배정 필요` 안내를 dev-safe 범위에서 구체화하기
...
- 막힘을 권한/placeholder/승인 필요/실제 버그로 구분
