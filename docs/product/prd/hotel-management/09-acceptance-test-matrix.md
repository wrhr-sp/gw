...
| `HOT-E2E-054` | protected Preview의 실제 Console client와 승인된 `previewadmin` credential | `/ui/console` 진입→custom login credential 제출→provider callback | Console이 만든 exact Auth Request로 `/ui/console/auth/callback` request와 Console 복귀를 확인하고 terminal 호텔 browser-binding cookie가 삭제됨; password·authRequest·CSRF·callback query는 출력하지 않으며 실패 시 CONTRACT 미실행 |
