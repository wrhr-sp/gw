...
- 신규 호텔은 `준비중`으로 저장한다.
- 운영활성화 필수조건: 기본정보, 활성 소유주 1명, 유효배정 사내 임직원 1명 이상, 객실 1개 이상, 유효한 호텔별 객실유형 1개 이상, 문의처.
...
- 호텔배정은 회사·호텔 범위를 정하는 필요조건일 뿐 조회·점검·이슈 기능을 자동 허용하지 않는다.
- 객실조회·점검수행·이슈등록·이슈작업·매출·설정·관리·문의답변은 각각 현재 DB 동적 기능권한과 개인 회수 우선을 함께 통과해야 한다.

### 하우스키핑

- 개인별 독립 사용자유형, 공용계정 금지.
- `사용자 승인`: 기간이 겹치는 복수 호텔 연결 가능.
- 배정 호텔 범위 안에서도 현재 DB 동적 조회·점검수행·이슈등록·이슈작업 권한을 통과한 행위만 허용한다. 배정이나 사용자유형만으로 기능권한을 자동 부여하지 않는다.
...
- 객실·유효한 호텔별 객실유형·문의처가 구현되기 전 활성화 요청은 누락항목을 모두 반환하고 상태를 변경하지 않는다. 가짜 활성화 성공을 금지한다. 점검관리자·체크리스트·점검일정은 호텔 활성화 필수조건으로 추가하지 않는다.
...
`HOTEL_STATUS_MANAGE`를 포함한 기능권한과 권한관리 자체권한은 최고관리자만 부여·회수한다. 역할명·관리자 표시명은 런타임 권한판정에 사용하지 않고, 최고관리자도 자기권한·상위권한·허용범위 밖 권한을 변경하지 못한다.
