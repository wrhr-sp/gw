...
평가순서는 `기본 거부 → company_id·호텔범위 교집합 → 회사 공통역할 허용 + 현재 호텔 역할 허용 + 회사 전체 사용자 추가 + 현재 호텔 사용자 추가 - 회사 전체 사용자 회수 - 현재 호텔 사용자 회수 → 자료유형 VIEW → DOWNLOAD → 자료상태·소유권`이다. 개인별 회수는 모든 역할·그룹·개인 추가허용보다 우선하며 호텔범위 밖이면 접근을 허용하지 않는다.

- 회사 공통역할·호텔별 역할·사용자그룹 기본권한 + 회사/호텔 범위 개인 추가·회수 예외.
- 역할명·직급명·부서명·`ADMIN` 문자열을 기능판정에 하드코딩하지 않는다.
- 최고관리자만 역할·기능권한과 권한관리 자체권한을 설정·부여·회수한다.
- 활성 최고관리자는 항상 정확히 두 명이어야 하며 한 명 또는 세 명이 되는 mutation을 DB가 차단한다.
- 최초 설정은 Preview bootstrap 운영자를 첫 최고관리자로 포함하고 그 운영자가 선택한 서로 다른 활성 사내 임직원 한 명을 두 번째 최고관리자로 지정하는 제한된 일회성 command만 허용한다. `0명 → 2명`을 한 transaction으로 처리하며 중간 1명 상태를 commit하지 않는다.
- 초기화 command는 현재 최고관리자 0명·승인된 bootstrap authority·두 계정의 활성 사내 임직원 상태·중요작업 재인증·승인참조·멱등키를 검증한다. 동일키 replay는 저장결과를 반환하고 다른 재호출·부분실패·동시 패자는 차단하며 성공 후 초기화 authority를 폐기하고 전후값·행위자·승인참조를 감사한다.
- 최고관리자 교체는 기존 한 명 해제와 신규 한 명 지정을 같은 transaction에서 처리한다.
- 최고관리자는 자기 자신을 교체할 수 없고 중요작업 재인증·사유·전후값·행위자·시각을 감사한다.
- 자기권한·상위권한·허용범위 밖 권한부여 금지.
- 배정·연결·권한기간 종료 즉시 회수.
- 권한 변경은 transaction 성공 뒤 다음 요청부터 재로그인 없이 적용하고 요청마다 DB 정본을 다시 읽는다.
- 변경 전후·사유·행위자·시각을 감사.
...
- `upload-complete`와 부모자료 연결은 업로드 생성자 일치만으로 허용하지 않는다. 각 요청에서 활성 session·사용자·회사·호텔배정·현재 기능권한·개인 회수 우선·부모자료 상태/version·동일 tenant를 다시 검증하고, 권한회수·배정만료·부모 완료잠금 이후에는 차단한다.
...
공통 감사 최소필드는 `event_code`, `actor_user_id`, `actor_type`, `company_id`, `hotel_id`, `resource_type`, `resource_id`, `before_summary`, `after_summary`, `reason`, `result`, `trace_id`, `occurred_at`이다. 권한 실패·범위 밖 접근·파일 보기/다운로드 실패도 기록한다. 감사 조회는 현재 DB 동적 감사조회권한과 회사·호텔 범위·개인 회수 우선을 통과한 사용자에게만 허용하고, 호텔 소유주·하우스키핑에는 감사원문을 노출하지 않는다.
