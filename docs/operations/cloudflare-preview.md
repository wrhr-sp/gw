...
5. 기존 Worker의 공개·인증 compatibility smoke가 성공한 뒤 신규 Worker를 배포한다.
   ...
   - account journey 실패 로그는 고정 allowlist의 비밀 비노출 stage code만 `PREVIEW_ACCOUNT_JOURNEY_FAILED_<STAGE>` 형식으로 출력한다. provider/API 응답 body, email·login ID·subject·session·credential 및 임의 오류문은 출력하지 않으며 allowlist 밖 code는 `UNCLASSIFIED`로 축약한다.
   - 기존 Worker compatibility의 Console pre-auth smoke에도 bootstrap subject를 해당 step에만 secret으로 주입한다. job-level 장기 노출이나 로그 출력은 금지하고, 누락 시 provider/Worker mutation 전에 fail-closed한다.
