...
최초 Preview 관리자 subject는 release 전에 ZITADEL에서 Active Human, 정확한 Preview organization, Ready MFA factor를 모두 충족해야 한다. MFA factor 등록은 bootstrap identity 보호 gate이며 현재 호텔 custom credential 화면의 입력은 짧은 ID·비밀번호까지다. ZITADEL 조직 정책이 로그인 MFA를 강제하면 호텔 custom login은 우회하지 않고 `AUTH_MFA_REQUIRED`로 중단한다. ZITADEL Session read-back에서 user 또는 password factor가 없으면 provider 계약상 해당 ID·비밀번호 검증이 성공하지 않은 것이므로 provider 장애가 아니라 `AUTH_CREDENTIALS_INVALID`로 처리한다. 이 credential mismatch에는 provider diagnostic stage header를 붙이지 않아 존재하지 않는 login ID와 동일한 외부 redirect·header 표면을 유지한다. subject·organization 불일치는 password factor보다 먼저 검사하고 provider unavailable로 안전 실패한다. 이 경우 ZITADEL 최초 관리자 비밀번호와 Preview protected environment의 `ZITADEL_PREVIEW_PASSWORD`를 운영자가 같은 값으로 맞춘 뒤 재검증하며 값 자체는 출력하거나 저장소에 남기지 않는다.

Preview protected environment에는 최초 관리자의 현재 비밀번호를 `ZITADEL_PREVIEW_PASSWORD` secret으로 저장한다. 값은 repository·문서·로그·artifact에 남기지 않고 release preflight와 `Verify hosted Preview Console credential and callback before contract` 단계에만 step-local로 주입한다. ZITADEL `GetSession` read-back은 공식 `GetSessionRequest.session_token` 계약에 따라 조회 대상 token을 REST query의 `sessionToken`으로 전달하고, `authenticated` API caller 계약에는 해당 session을 생성한 service PAT를 OAuth Bearer header로 전달한다. 아직 MFA 검증을 마치지 않은 사용자 session token을 API caller 인증으로 사용하지 않는다. request URL·query·session token은 로그·marker·오류 응답에 출력하지 않는다. 기존 Console pre-auth smoke는 비밀번호 없이 form·binding만 검증하며, 전용 credential 단계는 `/ui/console`에서 생성된 실제 Auth Request에 protected `PREVIEW_BOOTSTRAP_LOGIN_ID`와 secret password를 제출해 same-origin provider-owned `/ui/console/auth/callback`의 exact request·response path, response 2xx/3xx와 `error` 부재, 오류가 아닌 Console route, authenticated `GetMyUser` 200 read-back과 Console OIDC token의 issuer·audience·subject·expiry 일치, terminal 호텔 browser-binding 삭제까지 함께 확인해야 한다. callback response URL의 `code`·`state` query 보존 자체는 성공조건으로 삼지 않고, callback path와 이후 authenticated read-back·token identity를 결합해 완료를 증명한다. token claim은 Console storage key를 추정하지 않고 same-origin `/oauth/v2/token` 응답의 `id_token`을 메모리에서만 decode해 검증하며 token과 claim 값은 출력하거나 저장하지 않는다. 이 단계가 실패하면 CONTRACT를 실행하지 않는다. credential 실패는 `ZITADEL_CONSOLE_PREVIEW_CREDENTIAL_FAILED_<STAGE>`만 출력하며 stage는 `SUBMIT`, `CUSTOM_LOGIN_RESPONSE`, `CUSTOM_LOGIN_RESPONSE_STATUS`, `CUSTOM_LOGIN_REDIRECT_INVALID_FLOW`, `CUSTOM_LOGIN_REDIRECT_INVALID_CREDENTIALS`, `CUSTOM_LOGIN_REDIRECT_MFA_REQUIRED`, `CUSTOM_LOGIN_REDIRECT_RATE_LIMITED`, `CUSTOM_LOGIN_REDIRECT_UNAVAILABLE`, `CUSTOM_LOGIN_REDIRECT_OTHER`, `CUSTOM_LOGIN_PROVIDER_AUTH_REQUEST_INSPECT`, `CUSTOM_LOGIN_PROVIDER_LOGIN_SETTINGS`, `CUSTOM_LOGIN_PROVIDER_SESSION_CREATE`, `CUSTOM_LOGIN_PROVIDER_SESSION_READBACK`, `CUSTOM_LOGIN_PROVIDER_SESSION_READBACK_AUTHORIZATION`, `CUSTOM_LOGIN_PROVIDER_SESSION_READBACK_STATUS`, `CUSTOM_LOGIN_PROVIDER_SESSION_READBACK_JSON`, `CUSTOM_LOGIN_PROVIDER_SESSION_READBACK_SCHEMA`, `CUSTOM_LOGIN_PROVIDER_SESSION_READBACK_IDENTITY`, `CUSTOM_LOGIN_PROVIDER_ORGANIZATION_SETTINGS`, `CUSTOM_LOGIN_PROVIDER_AUTH_REQUEST_FINALIZE`, `CUSTOM_LOGIN_PROVIDER_NETWORK`, `CALLBACK_REQUEST`, `CALLBACK_RESPONSE`, `CALLBACK_RESPONSE_STATUS`, `CALLBACK_RESPONSE_ERROR`, `AUTHENTICATED_USER_RESPONSE`, `TERMINAL_LANDING`, `TOKEN_IDENTITY`, `TOKEN_RESPONSE`, `TOKEN_DECODE`, `TOKEN_ISSUER`, `TOKEN_AUDIENCE`, `TOKEN_SUBJECT`, `TOKEN_EXPIRY`, `COOKIE_CLEANUP`, `BROWSER_CLOSE`, `UNCLASSIFIED`로 제한한다. URL·callback query·credential·token·subject·cookie·provider body는 출력하지 않는다.
...
최초 Preview 비밀번호 재설정 메일은 ZITADEL 기본 링크 대신 아래 Preview custom URL template을 사용하며, 별도 승인된 one-time protected workflow에서만 요청한다. 실행 전에는 issuer discovery read-back과 issuer·organization·subject의 독립 승인 fingerprint, protected `ZITADEL_PREVIEW_APPROVED_EMAIL` 주소와 verified primary email의 timing-safe exact 비교, Active Human을 모두 확인해야 한다. PAT·raw subject·approved email·DB URL은 dependency install이 끝난 마지막 request step에만 주입하고 raw email은 로그·DB·artifact에 저장하지 않는다. 모든 Actions는 full commit SHA로 고정한다. `PREVIEW_BOOTSTRAP_APPROVAL_REF`별 durable 상태는 Preview DB에 `REQUESTING → REQUESTED`로 기록한다. 성공 replay는 외부 POST 0회로 종료하고 `REQUESTING`·`INDETERMINATE`는 provider audit와 mailbox 확인 전 자동 재발송하지 않는다. 승인 이메일 capture와 reset dispatch workflow는 accepted 요청 직후 저장소에서 삭제된 상태를 유지하며, 향후 재요청은 새 PR·독립 보안리뷰·명시 승인을 거쳐 다시 만든 뒤 즉시 삭제한다.
...
accepted reset dispatch 뒤에는 one-time workflow 삭제 cleanup PR을 즉시 병합하고 CI에서 capture·reset dispatch 파일이 모두 absent인지 확인한다. cleanup 전에는 one-shot 작업을 완료로 판정하지 않는다. 사용자가 새 비밀번호를 설정한 뒤에는 휴대폰에서도 GitHub protected Preview environment의 `ZITADEL_PREVIEW_PASSWORD`를 같은 값으로 갱신하고, 동일 subject hosted credential callback smoke를 다시 통과해야 한다. secret 갱신과 smoke 전에는 다음 Preview release를 실행하지 않는다.
...
7. 승인된 subject fingerprint·organization·고정 approval 참조를 확인하고, protected `PREVIEW_BOOTSTRAP_LOGIN_ID`를 공용 schema로 검증한 뒤 Preview 관리자와 회사 범위 `HOTEL_MANAGE`, `USER_READ`, `USER_CREATE`, `USER_SUSPEND`를 canonical row 전체 값으로 멱등 연결한다. 유효한 기존 canonical ID는 초기 `EXPAND`에서 유지한다. 신규 Worker 배포 후 `EXPAND_IDENTITY_LOCK` 또는 `CONTRACT`에서만 0023 history 계약을 적용하고, 승인된 이전 bootstrap alias에서 새 ID로 전환할 때 새 registry claim 선점·사용자 연결 변경·기존 session 회수·감사기록을 한 transaction으로 처리한다. 이전 registry claim은 tombstone으로 영구 보존한다.
