...
### 3. 현재 문서화/검증 기준은 Phase 13 관리자 콘솔 실사용 1차

현재 루트 문서와 handoff 는 Phase 13 관리자 콘솔 실사용 1차를 기준으로 맞추고 있다.

- 관리자 진입점은 일반 메뉴가 아니라 권한 있는 운영자에게만 보이는 CTA 기준으로 정리한다.
- 일반 사용자 기본 화면에서는 관리자 CTA 를 노출하지 않고, `/admin/*` 및 `/api/admin/*` 권한 guard 를 유지한다.
- 감사 전용 사용자는 `/admin/audit-logs` 읽기 흐름만 허용하고 다른 관리자 화면은 계속 차단한다.
- 최신 문서 근거로는 web 테스트 27건, shared 테스트 9건, api 테스트 57건, web typecheck, web build 통과 결과를 사용한다.
