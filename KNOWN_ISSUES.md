...
### 3. 현재 문서화/검증 기준은 Phase 27 노무 관리 1차

현재 루트 문서와 handoff 는 Phase 25 공통 work item 엔진과 Phase 26 HR lifecycle 기준 위에, 계약/연차/수당/고충/징계/사고/퇴사 skeleton 을 올리는 Phase 27 체인을 기준으로 맞춘다.

- 기본 일반 업무 흐름과 관리자 운영 흐름은 계속 유지하되, 현재 활성 정리 기준은 그 위에 labor issue 구조를 올리는 것이다.
- labor intake 상태와 업무 처리 상태를 같은 필드에 섞기 시작하면 노무 화면 언어가 복잡해질 수 있다.
- 본사 노무 담당 / HR / 지점 관리자 / 일반 직원 visibility 차이를 못 맞추면 같은 labor 이슈가 사용자마다 전혀 다른 뜻으로 읽힐 수 있다.
- 관련자와 열람권한을 같은 뜻으로 취급하면 비공개 노무 메모가 과하게 노출될 수 있다.
- 모바일 하단 탭을 늘리거나 `홈`/`메뉴`/PC sidebar 가 다른 구조를 가리키면 노무 진입 흐름이 화면마다 다른 제품처럼 보일 수 있다.
- 계약/징계/사고 자료를 metadata 단계와 원문 단계로 구분하지 않으면, 아직 없는 실민감 저장이 이미 가능한 것처럼 오해될 수 있다.
...
### 4. 현재 노무 관리 정리 단계에서 남아 있는 제품형 리스크

- 공통 상태와 labor intake/review 보조 상태를 분리하지 않으면 계약/수당/고충/징계/사고 이슈가 제각각 다른 흐름처럼 읽힐 수 있다.
- evidence/review/follow-up 구조가 work item 과 따로 놀면 노무 이슈 흐름이 다시 문서/메모/체크리스트로 분산될 수 있다.
- `confidentiality_level` 같은 민감도 구분 없이 설명을 시작하면 일반 직원용 안내와 restricted 노무 기록 경계가 흐려질 수 있다.
- 지점 관리자 visibility 와 본사 노무 담당 visibility 를 같은 수준으로 취급하면 호텔 지점 운영 동행 범위를 넘는 과노출 위험이 생길 수 있다.
- `홈`/`메뉴`/PC sidebar 에 labor entry 위치를 먼저 합의하지 않으면 구현 단계에서 메뉴가 비대해질 수 있다.
- 외부 노무/법무/급여 연동이나 실제 계약/징계/사고 원문 저장을 너무 일찍 문서에 섞어 쓰면 승인 게이트와 skeleton 초안이 구분되지 않을 수 있다.
