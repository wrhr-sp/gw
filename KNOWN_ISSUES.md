...
### 3. 현재 문서화/검증 기준은 Phase 21 실제 회사 설정 모델 1차

현재 루트 문서와 handoff 는 Phase 20 readiness 기준 위에, 회사 기본 설정/조직/직원/권한/근태·휴가 정책을 실제 회사 설정 묶음처럼 따라갈 수 있게 정리한 Phase 21 체인을 기준으로 맞춘다.
...
- 상태 안내는 offline, error, empty, forbidden 4축을 유지하되, Phase 21에서는 무엇이 어떤 회사 설정 결과인지/아직 skeleton 인지/승인 필요한지까지 같이 적는다.
...
### 4. 현재 실제 회사 설정 모델 정리 단계에서 남아 있는 제품형 리스크

- placeholder/skeleton 표현이 문서마다 다르면 일부 기능이 실제 회사 설정 저장·반영 완료처럼 오해될 수 있다.
- 회사 기본 설정, 조직/직원/권한, 정책 연결이 따로 놀면 대장이 실제 회사 모델 관점에서 구조를 잘못 이해할 수 있다.
- `/org`·`/employees` 일반 조회와 `/admin/users` 운영 검토 경계가 흔들리면 위험한 운영 액션이 일반 화면처럼 읽힐 수 있다.
- 출퇴근 정책 우선순위 설명과 휴가/근무 정책 설명이 다르면 같은 직원에게 어떤 정책이 적용되는지 오해할 수 있다.
...
- `/login` → `/dashboard` → `/org`·`/employees` → `/attendance`·`/leave` → `/approvals` → `/boards`·`/documents` → `/me` → `/admin/users`·`/admin/policies`·`/admin/audit-logs` → `/admin` 순서의 쉬운 확인 포인트가 문서마다 달라지면, 같은 저장소를 보고도 되는 것/아직 skeleton/승인 필요 판정이 흔들릴 수 있다.
