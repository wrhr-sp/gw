...
def final_body():
    return '부모 카드 결과와 show/runs/log, 관련 PR/CI/작업트리 상태를 확인하고 완료/미완료/사용자 결정 필요 사항을 쉬운 한국어로 최종 보고한다. 승인된 범위 안에서 누락된 안전 조치가 있으면 조치 후 보고하고, production DB/secret/DNS/유료/외부 공개/production migration/파괴적 정리는 승인 요청으로 보고한다.'
def steps(kind, hold):
...
def create(s,title,body,key):
    ...
    if s.key=='final': subscribe_report(tid)
