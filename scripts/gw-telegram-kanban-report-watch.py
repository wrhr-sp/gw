...
def build_blocked_message(conn: sqlite3.Connection, row: sqlite3.Row) -> str:
    ...
    return (
        ...
        f"- 쉬운 설명: 자동으로 고치기 어려운지, 대장 승인이 필요한지 먼저 나눠 본 보고입니다.\n\n"
        f"<b>카드 정보</b>\n"
        ...
        f"<b>확인한 근거</b>\n"
        f"- 막힌 이유/로그: {html.escape(reason)}\n\n"
        f"<b>다음 액션</b>\n"
        f"- 싱드 처리: {html.escape(next_action)}\n"
        "- 대장이 해줘야 할 것: 자동 조치 범위면 없습니다. 단, secret/운영 DB/DNS/유료/외부 공개/배포/migration이면 승인할 범위만 답해주시면 됩니다.\n\n"
        f"<b>처리 원칙</b>\n"
...
def build_completion_message(conn: sqlite3.Connection, row: sqlite3.Row, label: str, state: dict[str, Any]) -> str:
    ...
    if label == "Telegram 결과보고":
        ...
        user_action = "대장이 지금 해줄 일은 없습니다. 보고를 보고 추가 지시가 있으면 말씀해주시면 됩니다."
    elif label == "완료 보고":
        prefix = "✅"
        ...
        user_action = "대장이 지금 해줄 일은 없습니다. 후속 카드가 있으면 싱드가 자동으로 확인합니다."
    return (
        f"{prefix} <b>그룹웨어 {html.escape(label)}</b>\n\n"
        f"<b>결론</b>\n"
        f"- {html.escape(conclusion)}\n"
        "- 쉬운 설명: 작업이 실제로 끝났는지 카드 결과와 실행 근거를 확인해 보낸 보고입니다.\n\n"
        f"<b>카드 정보</b>\n"
        f"- 카드: <code>{html.escape(task_id)}</code>\n"
        f"- 제목: {html.escape(compact(title, 180))}\n"
        f"- 담당: {html.escape(assignee)}\n"
        ...
        f"- 후속: {html.escape(children)}\n\n"
        f"<b>확인한 근거</b>\n"
        f"- 실제 결과/로그: {html.escape(summary)}\n\n"
        f"<b>다음 액션</b>\n"
        f"- 대장이 해줘야 할 것: {html.escape(user_action)}\n"
        "- 싱드 처리: 남은 blocked/todo 카드가 있으면 싱드가 별도 확인합니다. 최종완료면 다음 작업 후보를 이어서 정리합니다.\n"
