...
| `ZITADEL_USER_PROVISIONER_TOKEN` | Preview 사용자 생성·중지 전용 최소권한 PAT | API secret |
| `ZITADEL_ORGANIZATION_ID` | 사용자 provisioning 대상 Preview organization | 환경변수 |
| `PREVIEW_BOOTSTRAP_APPROVAL_REF` | 보호된 Preview environment에 등록한 안정적인 최초 관리자 승인 티켓·결정 ID | 환경변수, 실행별 run ID 금지 |
