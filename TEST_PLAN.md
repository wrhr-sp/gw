...
## 통합설정 2차 비밀번호 미리보기 추가 검증 초점

- `docs/guides/unified-settings-secondary-password-preview-flow.md` 가 사용자가 실제로 따라갈 순서를 쉬운 한국어로 설명하는지 본다.
- 설정 관련 진입점에서 2차 비밀번호 게이트가 먼저 뜬다는 설명이 문서/테스트 근거와 같은 뜻인지 본다.
- 게이트, 설정 팝업, 변경 팝업이 모두 같은 4칸 네모형 PIN UI 기준을 쓴다고 적혀 있는지 본다.
- 숫자 4자리만 입력 가능, 입력 숫자 비노출, 채워진 칸/빈 칸 구분, 오류 문구가 PIN 아래에 표시된다는 설명이 빠지지 않는지 본다.
- 통합설정이 인증 후에도 항상 `기본 설정` 탭부터 열린다고 일관되게 적혀 있는지 본다.
- 관리자설정 우상단 `2차 비밀번호 변경` 버튼 제거와 `내정보 설정` 안 `2차 비밀번호 변경하기` 버튼 위치가 문서마다 충돌하지 않는지 본다.
- current preview UI 설명과 이후 서버/API/DB/감사 로그 설계를 같은 완료 문장처럼 섞지 않는지 본다.
- parent tester 검증 근거(28 files / 126 tests, typecheck, build)와 사용자 직접 확인 순서를 같은 수준의 근거처럼 섞지 않는지 본다.

## 관리자설정 접근권한/관리자 권한 분리 추가 검증 초점

- `docs/architecture/admin-settings-access-admin-permission-tabs-scope.md` 와 `docs/guides/admin-settings-access-admin-permission-tabs-handoff.md` 가 `접근권한` 과 `관리자 권한` 의미를 쉬운 한국어로 분리하는지 본다.
- 통합설정 상단 `기본 설정` / `관리자설정` 구조와 관리자설정 내부 `접근권한` / `관리자 권한` 2단 탭 구조가 서로 다른 책임으로 읽히는지 본다.
- `접근권한` 탭 문구에 조회·열람·진입 의미만 남고, 변경·부여·관리 의미가 섞이지 않는지 본다.
- `관리자 권한` 탭 문구에 변경·부여·관리 의미만 남고, 단순 조회·열람 의미가 섞이지 않는지 본다.
- 같은 주제라도 read 와 manage 를 나눠 적는지 본다. 예: 감사 로그 열람과 감사/보안 운영 관리가 같은 항목으로 합쳐지지 않는지 본다.
- 현재 `mobile-app-shell.tsx` preview 토글 재배치가 local state 범위를 넘지 않는지 본다.
- 실제 저장/API/DB/migration/audit log 운영 반영이 없는 점을 숨기지 않는지 본다.
- 설정 진입 2차 비밀번호 게이트, 통합설정 첫 탭 `기본 설정`, `내정보 설정` 안 `2차 비밀번호 변경하기`, 관리자설정 우상단 폐기 버튼 유지 기준이 회귀하지 않는지 본다.

## Phase 61 추가 검증 초점

- `docs/guides/phase-61-operational-db-provider-cost-approval-checklist.md` 가 운영 DB 관련 흩어진 문서를 한 번에 읽는 체크리스트 역할을 하는지 본다.
- `docs/guides/phase-61-operational-db-secret-cloudflare-rollback-runbook.md` 가 secret 보관, Workers 연결, rollback, restore smoke 기준을 한 문서에서 같이 보여 주는지 본다.
- `docs/guides/phase-61-operational-db-admin-handoff-checklist.md` 가 운영자가 실제로 먼저 확인할 파일 순서, 승인 질문, restore/rollback/smoke 인수인계 순서를 짧게 재사용 가능하게 묶는지 본다.
- 초기 권장 조합이 `Neon PostgreSQL 1개 + Cloudflare R2 + PostgreSQL Full Text Search` 로 일관되게 적혀 있는지 본다.
- Redis/Meilisearch 를 초기 비용 절감 때문에 보류한다는 문장이 `나중에 금지`가 아니라 `필요 시 중기 도입` 의미로 읽히는지 본다.
- provider 비교가 Neon 우선 / Supabase 대체 후보 수준으로 짧게 정리돼 있고, 근거 없이 다른 provider 를 현재 권장안처럼 섞지 않는지 본다.
- secret 입력 규칙이 채팅/로그/커밋 금지, git ignored `.secrets` 또는 승인된 secret store 허용으로 분명히 적혀 있는지 본다.
- `DATABASE_URL` / `DATABASE_URL_PREVIEW` / `DATABASE_URL_PRODUCTION` 해석 규칙과 `APP_ENV=preview` 기준이 앱 runtime 문서/코드에서 같은 뜻인지 본다.
- migration/seed 스크립트는 runtime 규칙과 별개로 production fallback 금지, preview 우선, `--allow-preview-fallback` 수동 범위만 허용이라는 정책을 같은 뜻으로 유지하는지 본다.
- `workers.dev` 또는 preview 배포는 preview DB 기준, 승인된 custom domain 또는 production 배포 후보는 production DB 기준 후보이며 production migration/seed 는 host 이름과 무관하게 `DATABASE_URL_PRODUCTION` 없으면 실행하지 않는다는 문장이 문서 전체에서 같은 뜻인지 본다.
- Cloudflare/Workers 첫 연결을 Worker secret 주입 + 현재 Neon serverless driver 경로 유지로 적고, Hyperdrive 를 초기 필수값처럼 과장하지 않는지 본다.
- provider 최종 선택, 실제 connection string 제공, 유료 리소스 생성·증설, production migration, 민감정보 원문 저장 확대, 외부기관 연동, DNS/custom domain, destructive 작업이 승인 게이트로 분리돼 있는지 본다.
- code rollback 과 DB rollback 을 분리해서 적고, DB rollback 을 destructive down migration 기본값처럼 쓰지 않는지 본다.
- `DB 연결 성공` 과 `권한/API guard·audit·backup/restore·rollback·live smoke 확인 완료` 가 같은 완료 문장으로 섞이지 않는지 본다.
- restore 후 최소 smoke 묶음(`/api/health`, `/login`, `/dashboard`, `/api/employees`, `/api/attendance/records`, `/api/leave/requests`, `/api/approvals/documents`, `/api/admin/audit-logs`, `/api/notifications`)이 문서와 runbook 에 같은 뜻으로 남아 있는지 본다.
- 승인 체크리스트, 다음 단계 체크리스트, 미정 사항 표를 대장이 바로 쓸 수 있는 쉬운 한국어로 유지하는지 본다.

## 관리자설정 2차 비밀번호 서버 검증 설계 추가 검증 초점

- `docs/architecture/admin-settings-secondary-password-server-policy.md` 가 현재 preview 상태와 운영 목표를 섞지 않고 분리해서 설명하는지 본다.
- 현재 `apps/web/app/_components/mobile-app-shell.tsx` 구현이 local state preview 라는 점과, 운영 버전이 서버 해시 저장 + verify session + 보호 API 재검증 구조라는 점이 문서에서 같은 뜻으로 유지되는지 본다.
- 관리자설정/고위험 변경 액션 보호와 일반 개인 설정 범위가 섞이지 않는지 본다.
- 등록/변경/분실/초기화 흐름이 각각 다른 정책으로 읽히고, 자동 self-service 분실 복구를 이미 지원하는 것처럼 과장하지 않는지 본다.
- 해시 저장, 실패 5회 잠금, 15분 재인증 세션, 5분 고위험 freshness 같은 권장값이 `확정 구현 완료`가 아니라 승인 전 초안이라는 점이 분명한지 본다.
- DB migration, hash 알고리즘, pepper secret, production rollout, 실데이터 변경이 별도 승인 게이트로 남아 있는지 본다.
- 후속 카드 순서(승인 정리 → 서버 구현 → 웹 연동 → 리뷰 → 테스트 → 문서화)가 handoff/TASKS/HANDOFF 문서에서 같은 뜻인지 본다.
...
- 접근 가능한 문서 공간 확인 → 파일 metadata 확인 → 업로드 준비 → 다운로드 준비 → 읽음 확인 흐름이 실제 업무 순서로 이어지는지 본다.
- 전사 문서함과 인사 전용 문서함 책임이 먼저 구분돼 보이는지 본다.
- `storageStatus` 와 문서 `status` 설명이 서로 같은 뜻처럼 섞이지 않는지 본다.
- private/missing space 차단, 회사 scope 차단, 권한 부족, raw storage internals 비노출이 route/API/UI 설명에서 같은 뜻인지 본다.
- empty / loading / error / forbidden / dev-safe 상태가 route 기준으로 분명히 보이고 서로 다른 뜻으로 읽히는지 본다.
- live 직접 재확인 근거와 local preview/build/test/release gate 대체 근거가 같은 수준으로 섞이지 않는지 본다.
...
- `/boards/board_notice` 와 `/boards/board_general` 이 notice-only 책임과 일반 글쓰기 책임을 먼저 구분해 보여 주는지 본다.
- `/boards/board_general` 에서 글 목록 → 글 작성 → 상세 진입이 자연스럽게 이어지는지 본다.
- `/posts/[postId]` 에서 댓글 작성, 읽음 확인 등록, forged/no-access 차단 확인이 실제 사용 순서로 읽히는지 본다.
- empty / loading / error / forbidden 상태가 route 기준으로 분명히 보이고 서로 다른 뜻으로 읽히는지 본다.
- `apps/api/test/auth-org.spec.ts` 의 accessible board/notice-only/forged 차단 근거와 웹 화면 설명이 같은 뜻인지 본다.
- live 직접 재확인 근거와 local preview/build/test/release gate 대체 근거가 같은 수준으로 섞이지 않는지 본다.
...
- 모바일 하단바 세부 UX에서는 `메뉴` → `홈` → `메신저` → `메일` → `알림` 순서, 아이콘 위/라벨 아래 세로 정렬, floating capsule 표현, active pill, 알림 배지 `0 숨김 / 1~99 / 99+`, 본문 하단 가림 방지가 실제 화면과 테스트에서 같은 뜻인지 본다.
...
- 협업 검토는 `/boards/board_notice`, `/boards/board_general`, `/posts/board_post_demo`, `/posts/board_post_notice_1`, `/documents` 같은 현재 예시 경로로 다시 눌러 보며 문서 설명과 실제 route 톤이 같은지 확인하는지
...
7-a. 모바일 하단바 — floating capsule 이 safe-area 위에 떠 있고, active pill 과 badge cap 이 흔들리지 않으며, 마지막 콘텐츠가 바 뒤로 숨지 않는지 본다.
