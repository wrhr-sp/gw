...
- 바로 확인 가능한 영역: `/management`, `/payroll`, `/payroll/me`, `/work-items/tax`, `/work-items/labor`, `/work-items/legal`, `/admin/audit-logs`, 관련 same-origin management/payroll/work-items/admin API, route/API/test 근거.
- 현재 기획 근거는 `apps/web/app/management/page.tsx`, `apps/web/app/payroll/page.tsx`, `apps/web/app/payroll/me/page.tsx`, `apps/web/app/work-items/work-items-config.ts`, `apps/web/app/work-items/tax/page.tsx`, `apps/web/app/work-items/labor/page.tsx`, `apps/web/app/work-items/legal/page.tsx`, `apps/web/app/admin/audit-logs/page.tsx`, `apps/api/src/app.ts`, `apps/api/test/auth-org.spec.ts`, `apps/api/test/work-items.spec.ts`, `packages/shared/src/contracts.ts` 다.
- 이번 Phase에서 가장 먼저 다시 고정할 것은 `/management` 운영 허브 lane, `/payroll` 운영 급여 lane, `/payroll/me` self-only lane, `tax/labor/legal` 모듈 lane, `/admin/audit-logs` 감사 read-only lane 책임 차이를 실제 운영 흐름으로 읽히게 만드는 것이다.
...
- 바로 확인 가능한 영역: `/login`, `/dashboard`, `/attendance`, `/leave`, `/approvals`, `/boards`, `/documents`, `/management`, `/work-items/branch`, `/admin/users`, `/admin/policies`, `/admin/audit-logs`, `/api/health`, `RUNBOOK.md`, `DEPLOYMENT.md`.
- 현재 기획 근거는 `apps/web` 일반 업무/운영/지점/감사 route, `apps/api/src/app.ts`, `packages/shared/src/admin-access.ts`, `packages/shared/src/contracts.ts`, `apps/api/test/auth-org.spec.ts`, `apps/api/test/work-items.spec.ts`, `apps/web/admin-preview-guard.test.ts`, `apps/web/work-items.test.tsx`, `apps/web/phase40-uat-package.test.tsx`, `docs/guides/phase-44-operator-runbook.md` 다.
- 이번 Phase에서 가장 먼저 다시 고정할 것은 일반 업무 레인, 운영 관리자 레인, 지점관리자 레인, 감사 read-only 레인을 실제 파일럿 UAT 순서와 기록 형식으로 읽히게 만드는 것이다.
- happy path, forbidden, empty, error, loading, mobile/PC 기록 포인트를 기능별로 나누되 같은 이슈 분류 언어로 묶어야 한다.
- production DB, 외부 IdP, 실급여/실신고, production backup/restore 실행, secret, 외부 SIEM/alerting, DNS/custom domain, 유료 리소스는 계속 restricted 승인 게이트다.
- 현재 Phase 49 사용자-facing 기준 live URL 메모는 `https://gw-web.wereheresp.workers.dev` 단일 host 로 정리한다. 다만 예전 Phase 문서에는 `werehere31` 흔적이 남아 있어 과거 기록과 현재 기준을 섞지 않게 주의한다.
- 새 운영 가이드 `docs/guides/phase-49-pilot-feedback-reflection-final-uat-regression-guide.md` 는 직원, 운영 관리자, 지점관리자, 감사 담당자용 추천 route, UAT 분류, 기록 포인트, 남은 승인 게이트를 한 문서로 묶는다.
- 최신 Phase 49 검증 기준은 focused API 15 files / 98 passed / 4 skipped, focused web 24 files / 103 tests passed, `pnpm --filter @gw/web typecheck`, `pnpm --filter @gw/web build`, `pnpm --filter @gw/web build:cf`, login-only redirect smoke baseline 이다.
- reviewer 가 처음 잡은 홈/company-scope vs branch-scope 혼동 문구는 `t_28b29919` 후속 수정으로 해소됐고, 문서에는 이 최신 분리 기준만 남긴다.
- live 직접 확인 근거는 singde unblock 메모와 `t_c353cc96` 완료 기록처럼 별도 출처로 남아 있으므로, local preview/build/test 통과 기록과 같은 뜻으로 합치지 않는다.
...
- 바로 확인 가능한 영역: `/login`, `/dashboard`, `/admin/users`, `/employees`, `/org`, `/management`, `/work-items/branch`, `/admin/audit-logs`.
- 현재 기획 근거는 `apps/web/app/page.tsx`, `apps/web/app/admin/users/page.tsx`, `apps/web/app/admin/users/admin-users-page-content.tsx`, `apps/web/app/employees/page.tsx`, `apps/web/app/org/page.tsx`, `apps/web/app/management/page.tsx`, `apps/api/src/app.ts`, `apps/api/src/lib/operational-org.ts`, `apps/api/test/auth-org.spec.ts`, `apps/web/work-items.test.tsx`, `packages/shared/src/contracts.ts`, `packages/shared/src/admin-access.ts` 다.
- 이번 Phase에서 가장 먼저 다시 고정할 것은 사용자 생성 → 역할/권한 diff → 조직/지점 확인 → 비활성/비밀번호 초기화 → 공통 post-login landing(`/dashboard`) 뒤 역할별 다음 레인 확인 순서를 운영자 UAT 문장으로 잠그는 것이다.
- COMPANY_ADMIN/HR_ADMIN/MANAGER/EMPLOYEE 는 `/dashboard`, AUDITOR 는 `/admin/audit-logs` 로 landing 하고, 그 뒤 HR 계정관리는 admin host `/admin/users`, 운영 관리는 general host `/management`, 감사는 admin host `/admin/audit-logs` 로 문서와 화면에서 절대 섞지 않는다.
- `/employees` 일반 조회와 `/admin/users` 운영 검토, `/org` 조직 읽기와 `/work-items/branch` 운영 레인도 서로 다른 책임으로 적는다.
- 실제 초대 발송, 외부 IdP/SSO, production 비밀번호/실데이터, DNS/custom domain, 유료 리소스, secret, migration 은 계속 승인 게이트다.
- 직전 Phase 45 tester 재검증 기준으로 focused API 15 files / 98 passed / 4 skipped, focused web 24 files / 100 passed, `pnpm --filter @gw/mobile typecheck`, `pnpm --filter @gw/web build` 가 모두 통과했다.
- blocked remediation fix/review/verify/recovery 이후 기준으로 focused web 재검증 24 files / 102 tests passed 가 추가 확인됐고, 문서는 이 최신 landing 기준을 우선 반영한다.
...
- 최신 parent 재검증 기준으로 shared/api/web 전체 테스트, typecheck, `pnpm check`, web build, `pnpm --filter @gw/web build:cf`, local preview curl smoke 가 모두 통과했다.
...
- 2026-06-17 자동 재수정 기준으로 `AUDITOR` 는 다시 `/admin/audit-logs` audit-only 흐름만 허용하고 `/management`·`/work-items/tax|labor|legal|branch` 경영업무 레인에서는 제외했다.
- 이번 Phase에서 가장 먼저 다시 고정할 것은 일반 직원 홈과 내부관리 허브를 서로 다른 책임 문맥으로 읽게 만들고, `/payroll` 과 `tax/labor/legal` 의 서로 다른 scope·approval gate 를 같은 말로 닫는 것이다.
...
- local admin-host preview smoke 기준 일반 host `/admin` 은 `/login`, admin host `/` 는 `/admin` 으로 redirect 되며 manifest split 도 다시 확인됐다.
- parent 최종 통합 보고 기준 live URL 은 `https://gw-web.wereheresp.workers.dev` 이고, 테스트 계정은 dev/test/UAT 전용 `admin / 1234` 다.
- 이번 Phase에서 가장 먼저 다시 고정할 것은 일반 직원 협업 레인(`/dashboard` → `/approvals` → `/boards` → `/documents`)과 운영 검토 레인(`/admin/policies`, `/admin/audit-logs`)을 서로 다른 책임 문맥으로 읽게 만드는 것이다.
- 공지 운영 책임과 일반 게시판 협업 책임, 문서 보관/열람과 외부 공유 미지원 상태, 결재 기안자 lane 과 승인자 lane 을 같은 말처럼 섞지 않게 유지해야 한다.
- self-approval 금지, replay 차단, forged post/read receipt 차단, private/missing space 차단은 단순 에러가 아니라 도입 기준 핵심 guardrail 로 적어야 한다.
- gap 이 큰 영역: 협업 기본 기능 4묶음을 `/dashboard` 기준 daily routine 으로 읽히게 하는 문구 부족, 운영 정책/감사 레인 분리 문구 부족, placeholder 와 내부 도입 가능 범위 구분 부족.
- 이번 문서의 목적은 제품 경계, 권한 경계, 협업 기본 기능 도입 언어를 한 세트의 내부 도입 기준으로 다시 맞추는 것이다.
